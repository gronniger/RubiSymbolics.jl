@testset "4.2.12 (e x)^m (a+b cos(c+d x^n))^p" begin
    ...
    @test_int [(e*x)^(n - 1)*(b*cos(c + d*x^n))^p, x, 3, -(((e*x)^n*(b*cos(c + d*x^n))^(1 + p)*Hypergeometric2F1(1/2, (1 + p)/2, (3 + p)/2, cos(c + d*x^n)^2)*sin(c + d*x^n))/(x^n*(b*d*e*n*(1 + p)*sqrt(sin(c + d*x^n)^2))))]
