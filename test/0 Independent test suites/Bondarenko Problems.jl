@testset "Bondarenko Problems" begin
    ...
    @test_int [sqrt(sinh(2*x)/cosh(x)), x, 5, (2*im*sqrt(2)*EllipticE(pi/4 - (im*x)/2, 2)*sqrt(sinh(x)))/sqrt(im*sinh(x)), (2*im*EllipticE(pi/4 - (im*x)/2, 2)*sqrt(sech(x)*sinh(2*x)))/sqrt(im*sinh(x))]
