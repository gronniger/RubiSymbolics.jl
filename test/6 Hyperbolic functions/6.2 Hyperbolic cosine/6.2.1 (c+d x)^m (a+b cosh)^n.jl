@testset "6.2.1 (c+d x)^m (a+b cosh)^n" begin
    ...
    @test_int [x^2/cosh(x)^(3/2) + x^2*sqrt(cosh(x)), x, 3, -8*x*sqrt(cosh(x)) - (16*im)*Elliptic.E((im/2)*x, 2) + (2*x^2*sinh(x))/sqrt(cosh(x))]
    ...
    @test_int [x^2/sech(x)^(3/2) - (x^2*sqrt(sech(x)))/3, x, 7, (-8*x)/(9*sech(x)^(3/2)) - ((16*im)/27)*sqrt(cosh(x))*Elliptic.F((im/2)*x, 2)*sqrt(sech(x)) + (16*sinh(x))/(27*sqrt(sech(x))) + (2*x^2*sinh(x))/(3*sqrt(sech(x)))]
