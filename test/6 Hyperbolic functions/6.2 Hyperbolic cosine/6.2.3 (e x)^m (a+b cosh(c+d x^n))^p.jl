@testset "6.2.3 (e x)^m (a+b cosh(c+d x^n))^p" begin
    ...
    @test_int [(e*x)^(n - 1)*(b*cosh(c + d*x^n))^p, x, 3, -(((e*x)^n*(b*cosh(c + d*x^n))^(1 + p)*Hypergeometric2F1(1/2, (1 + p)/2, (3 + p)/2, cosh(c + d*x^n)^2)*sinh(c + d*x^n))/(x^n*(b*d*e*n*(1 + p)*sqrt(-sinh(c + d*x^n)^2))))]
