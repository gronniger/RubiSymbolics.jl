@testset "7.1.2 (d x)^m (a+b arcsinh(c x))^n" begin
    ...
    @test_int [x^m*asinh(a*x)^2, x, 2, (x^(1 + m)*asinh(a*x)^2)/(1 + m) - (2*a*x^(2 + m)*asinh(a*x)*Hypergeometric2F1(1/2, (2 + m)/2, (4 + m)/2, (-a^2)*x^2))/(2 + 3*m + m^2) + (2*a^2*x^(3 + m)*HypergeometricPFQ([1, 3/2 + m/2, 3/2 + m/2], [2 + m/2, 5/2 + m/2], (-a^2)*x^2))/(6 + 11*m + 6*m^2 + m^3)]
    @test_int [x^m*asinh(a*x)^1, x, 2, (x^(1 + m)*asinh(a*x))/(1 + m) - (a*x^(2 + m)*Hypergeometric2F1(1/2, (2 + m)/2, (4 + m)/2, -(a^2*x^2)))/(2 + 3*m + m^2)]
