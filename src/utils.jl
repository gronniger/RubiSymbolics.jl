...
IntHide(u_,x_Symbol) :=
  Block([DOLLARShowSteps=false,DOLLARStepCounter=nothing], Int(u,x))
...
EveryQ(func_,lst_) :=
  Catch(Scan(Function(If(func(Slot(1)),nothing,Throw(false))),lst);true)
...
IntegersQ(x::Integer...) = true;
IntegersQ(x::Any...) = false;
...
HalfIntegerQ(u__) := Scan(Function(If(Head(Slot(1))===Rational && Denominator(Slot(1))==2,nothing,Return(false))),[u])===nothing
...
FractionQ(x::Rational...) = true;
FractionQ(x::Any...) = false;
...
RationalQ(u__) := Scan(Function(If(IntegerQ(Slot(1)) || Head(Slot(1))===Rational,nothing,Return(false))),[u])===nothing
...
ComplexNumberQ(::Complex) = true;
ComplexNumberQ(::Any) = false;
...
FractionOrNegativeQ(u__) := Scan(Function(If(FractionQ(Slot(1)) || IntegerQ(Slot(1)) && Slot(1)<0,nothing,Return(false))),[u])===nothing
...
IntegerPowerQ = Function(Head(Slot(1))===Power && IntegerQ(Slot(1)[2]));
...
FractionalPowerQ = Function(Head(Slot(1))===Power && Head(Slot(1)[2])===Rational);
...
FractionalPowerFreeQ(u_) :=
  ...
  If(FractionalPowerQ(u) && Not(AtomQ(u[1])),
    false,
  Catch(Scan(Function(If(FractionalPowerFreeQ(Slot(1)),nothing,Throw(false))),u);true)))
...
ComplexFreeQ(u_) :=
  ...
  Scan(Function(If(ComplexFreeQ(Slot(1)),nothing,Return(false))),u)===nothing)
...
DOLLARInverseTrigFunctions = [ArcSin, ArcCos, ArcTan, ArcCot, ArcSec, ArcCsc];
...
DOLLARInverseHyperbolicFunctions = [ArcSinh, ArcCosh, ArcTanh, ArcCoth, ArcSech, ArcCsch];
...
DOLLARCalculusFunctions = [D, Integrate, Sum, Product, Int, Unintegrable, CannotIntegrate, Dif, Subst];
CalculusQ(u_) := MemberQ(DOLLARCalculusFunctions, If(AtomQ(u),u,Head(u)))


DOLLARStopFunctions = [Hold, HoldForm, Defer, Pattern, If, Int, Unintegrable, CannotIntegrate];
StopFunctionQ(u_) :=
  If(AtomQ(Head(u)),
...
TrigHyperbolicFreeQ(u_,x_Symbol) :=
  ...
  If(TrigQ(u) || HyperbolicQ(u) || CalculusQ(u),
    FreeQ(u,x),
  Catch(Scan(Function(If(TrigHyperbolicFreeQ(Slot(1),x),nothing,Throw(false))),u);true)))
...
InverseFunctionFreeQ(u_,x_Symbol) :=
  ...
  If(InverseFunctionQ(u) || CalculusQ(u) || Head(u)===Hypergeometric2F1 || Head(u)===AppellF1,
    FreeQ(u,x),
  Catch(Scan(Function(If(InverseFunctionFreeQ(Slot(1),x),nothing,Throw(false))),u);true)))
...
CalculusFreeQ(u_,x_) :=
  ...
  If(CalculusQ(u) && u[2]===x || HeldFormQ(u),
    false,
  Catch(Scan(Function(If(CalculusFreeQ(Slot(1),x),nothing,Throw(false))),u); true)))
...
PolyQ(u_,x_Symbol^n_Integer) :=
  (If(PolynomialQ(u,x),
    PolynomialQ(u,x^n),
  With([v=Together(u)], PolynomialQ(v,x) && PolynomialQ(v,x^n))) <-- n>0)
...
PolyQ(u_,x_Symbol^v_) :=
  ...
  With([w=Together(u)], Quiet(PolynomialQ(w,x^v))===true && FreeQ(CoefficientList(w,x^v),x))) <-- NonsumQ(v) && FreeQ(v,x))
...
BinomialQ(u_,x_Symbol) :=
  If(ListQ(u),
    Catch(Scan(Function(If(Not(BinomialQ(Slot(1),x)),Throw(false))),u); true),
...
BinomialQ(u_,x_Symbol,n_) :=
  If(ListQ(u),
    Catch(Scan(Function(If(Not(BinomialQ(Slot(1),x,n)),Throw(false))),u); true),
  Function(ListQ(Slot(1)) && Slot(1)[3]===n)(BinomialParts(u,x)))
...
TrinomialQ(u_,x_Symbol) :=
  If(ListQ(u),
    Catch(Scan(Function(If(Not(TrinomialQ(Slot(1),x)),Throw(false))),u); true),
...
GeneralizedBinomialQ(u_,x_Symbol) :=
  If(ListQ(u),
    Catch(Scan(Function(If(Not(GeneralizedBinomialQ(Slot(1),x)),Throw(false))),u); true),
...
GeneralizedTrinomialQ(u_,x_Symbol) :=
  If(ListQ(u),
    Catch(Scan(Function(If(Not(GeneralizedTrinomialQ(Slot(1),x)),Throw(false))),u); true),
...
NiceSqrtAuxQ(u_) :=
  ...
  If(SumQ(u),
    Function(NonsumQ(Slot(1)) && NiceSqrtAuxQ(Slot(1)))(Simplify(u)),
  false))))
...
PerfectSquareQ(u_) :=
  ...
  If(SumQ(u),
    Function(NonsumQ(Slot(1)) && PerfectSquareQ(Slot(1)))(Simplify(u)),
  false))))
...
SumSimplerAuxQ(u_,v_) :=
  ...
  (RationalQ(Rest(v)) || SumSimplerAuxQ(u,Rest(v))) <-- SumQ(v))

SumSimplerAuxQ(u_,v_) :=
  (SumSimplerAuxQ(First(u),v) || SumSimplerAuxQ(Rest(u),v) <-- SumQ(u))
...
CancelCommonFactors(u_,v_) :=
  If(ProductQ(u),
    If(ProductQ(v),
      ...
      Function([First(u)*Slot(1)[1],Slot(1)[2]])(CancelCommonFactors(Rest(u),v))),
    If(MemberQ(u,v),
...
TrinomialParts(u_,x_Symbol) :=
  If(PolynomialQ(u,x),
    ...
    If(Length(lst)<3 || EvenQ(Length(lst)) || EqQ(lst[(Length(lst)+1)/2],0),
      false,
    Catch(
      Scan(Function(If(EqQ(Slot(1),0),nothing,Throw(false))),Drop(Drop(Drop(lst,[(Length(lst)+1)/2]),1),-1));
...
GeneralizedBinomialDegree(u_,x_Symbol) :=
  Function(Slot(1)[3]-Slot(1)[4])(GeneralizedBinomialParts(u,x))
...
GeneralizedBinomialParts(a_'*x_^q_'+b_'*x_^n_',x_Symbol) :=
  ([a,b,n,q] <-- FreeQ([a,b,n,q],x) && PosQ(n-q))

GeneralizedBinomialParts(a_*u_,x_Symbol) :=
  (With([lst=GeneralizedBinomialParts(u,x)],
  ([a*lst[1], a*lst[2], lst[3], lst[4]] <-- ListQ(lst))) <-- FreeQ(a,x))

GeneralizedBinomialParts(x_^m_'*u_,x_Symbol) :=
  (With([lst=GeneralizedBinomialParts(u,x)],
  ([lst[1], lst[2], m+lst[3], m+lst[4]] <-- ListQ(lst) && NeQ(m+lst[3],0) && NeQ(m+lst[4],0))) <-- FreeQ(m,x))

GeneralizedBinomialParts(x_^m_'*u_,x_Symbol) :=
  (With([lst=BinomialParts(u,x)],
  ([lst[1], lst[2], m+lst[3], m] <-- ListQ(lst) && NeQ(m+lst[3],0))) <-- FreeQ(m,x))
...
GeneralizedTrinomialDegree(u_,x_Symbol) :=
  Function(Slot(1)[4]-Slot(1)[5])(GeneralizedTrinomialParts(u,x))
...
GeneralizedTrinomialParts(a_'*x_^q_'+b_'*x_^n_'+c_'*x_^r_',x_Symbol) :=
  ([a,b,c,n,q] <-- FreeQ([a,b,c,n,q],x) && EqQ(r,2*n-q))

GeneralizedTrinomialParts(a_*u_,x_Symbol) :=
  (With([lst=GeneralizedTrinomialParts(u,x)],
  ([a*lst[1], a*lst[2], a*lst[3], lst[4], lst[5]] <-- ListQ(lst))) <-- FreeQ(a,x))
...
GeneralizedTrinomialParts(x_^m_'*u_,x_Symbol) :=
  (With([lst=GeneralizedTrinomialParts(u,x)],
  ([lst[1], lst[2], lst[3], m+lst[4], m+lst[5]] <-- ListQ(lst) && NeQ(m+lst[4],0) && NeQ(m+lst[5],0))) <-- FreeQ(m,x))

GeneralizedTrinomialParts(x_^m_'*u_,x_Symbol) :=
  (With([lst=TrinomialParts(u,x)],
  ([lst[1], lst[2], lst[3], m+lst[4], m] <-- ListQ(lst) && NeQ(m+lst[4],0))) <-- FreeQ(m,x))
...
NumericFactor(u_) :=
  ...
  If(SumQ(u),
    If(LeafCount(u)<50,             #= Eliminate this kludge! =#
      Function(If(SumQ(Slot(1)), 1, NumericFactor(Slot(1))))(ContentFactor(u)),
...
NonnumericFactors(u_) :=
  ...
  If(SumQ(u),
    If(LeafCount(u)<50,             #= Eliminate this kludge! =#
      Function(If(SumQ(Slot(1)), u, NonnumericFactors(Slot(1))))(ContentFactor(u)),
    With([n=NumericFactor(u)],
    Map(Function(Slot(1)/n),u))),
  u))))
...
RemoveContentAux(a_^m_*u_'+b_*v_',x_Symbol) :=
  ...
  RemoveContentAux(u-a^(1-m)*v,x)) <-- IntegersQ(a,b) && a+b==0 && RationalQ(m))


RemoveContentAux(a_^m_'*u_'+a_^n_'*v_',x_Symbol) :=
  (RemoveContentAux(u+a^(n-m)*v,x) <-- FreeQ(a,x) && RationalQ(m,n) && n-m>=0)


RemoveContentAux(a_^m_'*u_'+a_^n_'*v_'+a_^p_'*w_',x_Symbol) :=
  (RemoveContentAux(u+a^(n-m)*v+a^(p-m)*w,x) <-- FreeQ(a,x) && RationalQ(m,n,p) && n-m>=0 && p-m>=0)
...
FreeFactors(u_,x_) :=
  If(ProductQ(u),
    Map(Function(If(FreeQ(Slot(1),x),Slot(1),1)),u),
  If(FreeQ(u,x),
    u,
  1))
...
NonfreeFactors(u_,x_) :=
  If(ProductQ(u),
    Map(Function(If(FreeQ(Slot(1),x),1,Slot(1))),u),
  If(FreeQ(u,x),
    1,
  u))
...
FreeTerms(u_,x_) :=
  If(SumQ(u),
    Map(Function(If(FreeQ(Slot(1),x),Slot(1),0)),u),
  If(FreeQ(u,x),
    u,
  0))
...
NonfreeTerms(u_,x_) :=
  If(SumQ(u),
    Map(Function(If(FreeQ(Slot(1),x),0,Slot(1))),u),
  If(FreeQ(u,x),
    0,
  u))
...
LinearQ(u_,x_Symbol) :=
  If(ListQ(u),
    Catch(Scan(Function(If(PolyQ(Slot(1),x,1),nothing,Throw(false))),u); true),
  PolyQ(u,x,1))
...
QuadraticProductQ(u_,x_Symbol) :=
  ProductQ(NonfreeFactors(u,x)) && Catch(
	Scan(Function(If(MatchQ(Slot(1),(Pm_^m_' <-- PolyQ(Pm,x) && Expon(Pm,x)<=2 && IntegerQ(m))), nothing, Throw(false))),NonfreeFactors(u,x));
    true)
...
QuadraticQ(u_,x_Symbol) :=
  If(ListQ(u),
    Catch(Scan(Function(If(Not(QuadraticQ(Slot(1),x)),Throw(false))),u); true),
...
MonomialQ(u_,x_Symbol) :=
  If(ListQ(u),
    Catch(Scan(Function(If(MonomialQ(Slot(1),x),nothing,Throw(false))),u); true),
...
MinimumMonomialExponent(u_,x_Symbol) :=
  ...
  Scan(Function(If(PosQ(n-MonomialExponent(Slot(1),x)),n=MonomialExponent(Slot(1),x))),u);
  n)
...
LinearMatchQ(u_,x_Symbol) :=
  If(ListQ(u),
    Catch(Scan(Function(If(Not(LinearMatchQ(Slot(1),x)),Throw(false))),u); true),
...
PowerOfLinearMatchQ(u_,x_Symbol) :=
  If(ListQ(u),
    Catch(Scan(Function(If(Not(PowerOfLinearMatchQ(Slot(1),x)),Throw(false))),u); true),
...
QuadraticMatchQ(u_,x_Symbol) :=
  If(ListQ(u),
    Catch(Scan(Function(If(Not(QuadraticMatchQ(Slot(1),x)),Throw(false))),u); true),
...
CubicMatchQ(u_,x_Symbol) :=
  If(ListQ(u),
    Catch(Scan(Function(If(Not(CubicMatchQ(Slot(1),x)),Throw(false))),u); true),
...
BinomialMatchQ(u_,x_Symbol) :=
  If(ListQ(u),
    Catch(Scan(Function(If(Not(BinomialMatchQ(Slot(1),x)),Throw(false))),u); true),
...
GeneralizedBinomialMatchQ(u_,x_Symbol) :=
  If(ListQ(u),
    Catch(Scan(Function(If(Not(GeneralizedBinomialMatchQ(Slot(1),x)),Throw(false))),u); true),
...
TrinomialMatchQ(u_,x_Symbol) :=
  If(ListQ(u),
    Catch(Scan(Function(If(Not(TrinomialMatchQ(Slot(1),x)),Throw(false))),u); true),
...
GeneralizedTrinomialMatchQ(u_,x_Symbol) :=
  If(ListQ(u),
    Catch(Scan(Function(If(Not(GeneralizedTrinomialMatchQ(Slot(1),x)),Throw(false))),u); true),
...
QuotientOfLinearsMatchQ(u_,x_Symbol) :=
  If(ListQ(u),
    Catch(Scan(Function(If(Not(QuotientOfLinearsMatchQ(Slot(1),x)),Throw(false))),u); true),
...
PerfectPowerTest(u_,x_Symbol) :=
  If(PolynomialQ(u,x),
    ...
    If(lst[1]===[1,1],
      lst=Rest(lst));
    Scan(Function(gcd=GCD(gcd,Slot(1)[2])),lst);
    If(gcd>1,
      Scan(Function(v=v*Slot(1)[1]^(Slot(1)[2]/gcd)),lst);
      Expand(v)^gcd,
    false)),
  false)
...
RationalFunctionQ(u_,x_Symbol) :=
  ...
  If(ProductQ(u) || SumQ(u),
    Catch(Scan(Function(If(Not(RationalFunctionQ(Slot(1),x)),Throw(false))),u);true),
  false)))
...
RationalFunctionFactors(u_,x_Symbol) :=
  If(ProductQ(u),
    Map(Function(If(RationalFunctionQ(Slot(1),x),Slot(1),1)),u),
...
NonrationalFunctionFactors(u_,x_Symbol) :=
  If(ProductQ(u),
    Map(Function(If(RationalFunctionQ(Slot(1),x),1,Slot(1))),u),
  If(RationalFunctionQ(u,x),
    1,
  u))
...
RationalFunctionExpand(u_*v_^n_,x_Symbol) :=
  ...
  If(SumQ(w),
    Map(Function(Slot(1)*v^n),w),
  w*v^n)) <-- FractionQ(n) && v!==x)
...
AlgebraicFunctionQ(u_,x_Symbol,flag_:False) :=
  ...
  If(ProductQ(u) || SumQ(u),
    Catch(Scan(Function(If(Not(AlgebraicFunctionQ(Slot(1),x,flag)),Throw(false))),u);true),
  If(ListQ(u),
    If(u===[],
      true,
...
QuotientOfLinearsQ(u_,x_Symbol) :=
  If(ListQ(u),
    Catch(Scan(Function(If(Not(QuotientOfLinearsQ(Slot(1),x)),Throw(false))),u); true),
  QuotientOfLinearsP(u,x) && Function(NeQ(Slot(1)[2],0) && NeQ(Slot(1)[4],0))(QuotientOfLinearsParts(u,x)))


QuotientOfLinearsP(a_*u_,x_) :=
  (QuotientOfLinearsP(u,x) <-- FreeQ(a,x))
...
QuotientOfLinearsParts(a_*u_,x_) :=
  (Apply(Function([a*Slot(1), a*Slot(2), Slot(3), Slot(4)]), QuotientOfLinearsParts(u,x)) <-- FreeQ(a,x))

QuotientOfLinearsParts(a_+u_,x_) :=
  (Apply(Function([Slot(1)+a*Slot(3), Slot(2)+a*Slot(4), Slot(3), Slot(4)]), QuotientOfLinearsParts(u,x)) <-- FreeQ(a,x))

QuotientOfLinearsParts(1/u_,x_) :=
  Apply(Function([Slot(3), Slot(4), Slot(1), Slot(2)]), QuotientOfLinearsParts(u,x))

QuotientOfLinearsParts(u_,x_) :=
  ([Coefficient(u,x,0), Coefficient(u,x,1), 1, 0] <-- LinearQ(u,x))

QuotientOfLinearsParts(u_/v_,x_) :=
  ([Coefficient(u,x,0), Coefficient(u,x,1), Coefficient(v,x,0), Coefficient(v,x,1)] <-- LinearQ(u,x) && LinearQ(v,x))
...
SubstForFractionalPowerQ(u_,v_,x_Symbol) :=
  ...
  Catch(Scan(Function(If(Not(SubstForFractionalPowerQ(Slot(1),v,x)),Throw(false))),u);true)))
...
SubstForFractionalPowerAuxQ(u_,v_,x_) :=
  ...
  If(FractionalPowerQ(u) && EqQ(u[1],v),
    true,
  Catch(Scan(Function(If(SubstForFractionalPowerAuxQ(Slot(1),v,x),Throw(true))),u);false)))
...
FractionalPowerOfQuotientOfLinears(u_,n_,v_,x_) :=
  ...
  Catch(Module([lst=[n,v]],
    Scan(Function(If(AtomQ(lst=FractionalPowerOfQuotientOfLinears(Slot(1),lst[1],lst[2],x)),Throw(false))),u);
    lst)))))
...
InverseFunctionOfQuotientOfLinears(u_,x_Symbol) :=
  ...
  Module([tmp],
  Catch(
    Scan(Function(If(Not(AtomQ(tmp=InverseFunctionOfQuotientOfLinears(Slot(1),x))),Throw(tmp))),u);
    false))))
...
SubstForFractionalPower(u_,v_,n_,w_,x_Symbol) :=
  ...
  If(FractionalPowerQ(u) && EqQ(u[1],v),
    x^(n*u[2]),
  Map(Function(SubstForFractionalPower(Slot(1),v,n,w,x)),u)))
...
SubstForInverseFunction(u_,v_,w_,x_Symbol) :=
  ...
  If(Head(u)===Head(v) && EqQ(u[1],v[1]),
    x,
  Map(Function(SubstForInverseFunction(Slot(1),v,w,x)),u)))
...
FactorAbsurdNumber(m_) :=
  If(RationalQ(m),
    FactorInteger(m),
  If(PowerQ(m),
    Map(Function([Slot(1)[1], Slot(1)[2]*m[2]]),FactorInteger(m[1])),
  CombineExponents(Sort(Flatten(Map(FactorAbsurdNumber,Apply(List,m)),1), Function(Slot(1)[1]<Slot(2)[1])))))
...
AbsurdNumberGCDList(lst1_,lst2_) :=
  If(lst1===[],
    Apply(Times,Map(Function(Slot(1)[1]^Min(Slot(1)[2],0)),lst2)),
  If(lst2===[],
    Apply(Times,Map(Function(Slot(1)[1]^Min(Slot(1)[2],0)),lst1)),
...
NormalizeIntegrandAux(u_,x_Symbol) :=
  If(SumQ(u),
    Map(Function(NormalizeIntegrandAux(Slot(1),x)),u),
  If(ProductQ(MergeMonomials(u,x)),
    Map(Function(NormalizeIntegrandFactor(Slot(1),x)),MergeMonomials(u,x)),
...
NormalizeIntegrandFactor(u_,x_Symbol) :=
  ...
  If(PowerQ(u) && FreeQ(u[2],x),
    ...
    deg=u[2];
    If(IntegerQ(deg) && SumQ(bas) && EveryQ(Function(MonomialQ(Slot(1),x)),bas),
      min=MinimumMonomialExponent(bas,x);
      x^(min*deg)*Map(Function(Simplify(Slot(1)/x^min)),bas)^deg,
    bas^deg),
  If(PowerQ(u) && FreeQ(u[1],x),
    u[1]^NormalizeIntegrandFactorBase(u[2],x),
  bas=NormalizeIntegrandFactorBase(u,x);
  If(SumQ(bas) && EveryQ(Function(MonomialQ(Slot(1),x)),bas),
    min=MinimumMonomialExponent(bas,x);
    x^min*Map(Function(Slot(1)/x^min),bas),
  bas))))


NormalizeIntegrandFactorBase(x_^m_'*u_,x_Symbol) :=
  (NormalizeIntegrandFactorBase(Map(Function(x^m*Slot(1)),u),x) <-- FreeQ(m,x) && SumQ(u))
...
NormalizeIntegrandFactorBase(u_,x_Symbol) :=
  ...
  If(TrinomialQ(u,x),
    ...
    ExpandToSum(u,x)),
  If(ProductQ(u),
    Map(Function(NormalizeIntegrandFactor(Slot(1),x)),u),
...
NormalizeSumFactors(u_) :=
  If(AtomQ(u) || StopFunctionQ(u),
    u,
  If(ProductQ(u),
    Function(Slot(1)[1]*Slot(1)[2])(SignOfFactor(Map(NormalizeSumFactors,u))),
...
MergeMonomials(u_'*(a_'+b_'*x_)^m_'*(c_'+d_'*x_)^n_',x_Symbol) :=
  (u*b^m/d^m*(c+d*x)^(m+n) <-- FreeQ([a,b,c,d],x) && IntegerQ(m) && EqQ(b*c-a*d,0))
...
MergeMonomials(u_'*(a_'+b_'*x_)^m_'*(c_'*(a_'+b_'*x_)^n_')^p_,x_Symbol) :=
  (u*(c*(a+b*x)^n)^(m/n+p)/c^(m/n) <-- FreeQ([a,b,c,m,n,p],x) && IntegerQ(m/n))
...
SubstForExpn(u_,v_,w_) :=
  ...
  If(AtomQ(u),
    u,
  Map(Function(SubstForExpn(Slot(1),v,w)),u)))
...
Simp((e_'*(a_+b_)^r_')^p_'*(c_+d_)^q_') :=
  ...
  If(IntegerQ(p) || GtQ(e,0) && GtQ(u,0) && GtQ(r,0),
    e^p*u^(p*r),
  e^IntPart(p)*u^(r*IntPart(p))*(e*(a+b)^r)^FracPart(p)/(c+d)^(r*FracPart(p)))) <-- IntegerQ(r) && EqQ(p*r+q,0))
...
Simp((g_'*(a_+b_)^s_')^p_'*(c_+d_)^q_'*(e_+f_)^r_') :=
  ...
  g^IntPart(p)*u^(s*IntPart(p))*(g*(a+b)^s)^FracPart(p)/((c+d)^(s*FracPart(p))*(e+f)^(s*FracPart(p)))))))) <-- IntegerQ(s) && EqQ(p*s+q,0) && EqQ(p*s+r,0))


Simp((u_^q_'*v_^r_')^p_'*u_^pq_'*v_^pr_') :=
  (If(IntegerQ(p),
    1,
  (u^q*v^r)^FracPart(p)/(u^(q*FracPart(p))*v^(r*FracPart(p)))) <-- IntegersQ(q,r) && EqQ(pq+p*q,0) && EqQ(pr+p*r,0))
...
Simp((a_+b_)^p_'*(c_+d_)^q_*(e_+f_)^q_) :=
  ((a/c^2)^p*(c+d)^(p+q)*(e+f)^(p+q) <-- IntegerQ(p) && EqQ(e,c) && EqQ(f,-d) && EqQ(b*c^2+a*d^2,0))


Simp((a_+b_)^p_*(c_+d_)^p_) :=
  ((a^2-b^2)^p <-- EqQ(a,c) && EqQ(b,-d) && GtQ(a,0))
...
SimpHelp(u_,x_) :=
  ...
  If(ProductQ(u),
    ...
    w=If(ProductQ(w), Map(Function(SimpHelp(Slot(1),x)),w), SimpHelp(w,x));
  ...
  If(SumQ(u),
    ...
    Module([v=0,w=0],
    Scan(Function(If(FreeQ(Slot(1),x),v=Slot(1)+v,w=Slot(1)+w)),u);
    v=SmartSimplify(v);
    w=If(SumQ(w), Map(Function(SimpHelp(Slot(1),x)),w), SimpHelp(w,x));
    v+w)))),
  If(TrigQ(u),
...
FractionalPowerOfSquareQ(u_) :=
  ...
  Module([tmp],
  Catch(
    Scan(Function(If(Not(FalseQ(tmp=FractionalPowerOfSquareQ(Slot(1)))),Throw(tmp))),u);
    false))))
...
FractionalPowerSubexpressionQ(u_,v_,w_) :=
  ...
  Catch(Scan(Function(If(FractionalPowerSubexpressionQ(Slot(1),v,w),Throw(true))),u); false)))
...
FixSimplify(u_'*Complex(0,a_)*(v_'*Complex(0,b_)+w_)^n_') :=
  ((-1)^((n+1)/2)*a*u*FixSimplify((b*v-Complex(0,1)*w)^n) <-- OddQ(n))
...
FixSimplify(w_'*u_^m_'*v_^n_) :=
  ...
  (FixSimplify(w*z^GCD(m,n)) <-- AbsurdNumberQ(z) || SqrtNumberSumQ(z))) <-- RationalQ(m) && FractionQ(n) && SqrtNumberSumQ(u) && SqrtNumberSumQ(v) && GtQ(u,0) && GtQ(v,0))


FixSimplify(w_'*u_^m_'*v_^n_) :=
  (With([z=Simplify(u^(m/GCD(m,-n))*v^(n/GCD(m,-n)))],
  (FixSimplify(w*z^GCD(m,-n)) <-- AbsurdNumberQ(z) || SqrtNumberSumQ(z))) <-- RationalQ(m) && FractionQ(n) && SqrtNumberSumQ(u) && SqrtNumberSumQ(1/v) && GtQ(u,0) && GtQ(v,0))
...
FixSimplify(w_'*u_^m_'*v_^n_) :=
  (With([z=Simplify((-u)^(m/GCD(m,n))*v^(n/GCD(m,n)))],
  (FixSimplify(-w*z^GCD(m,n)) <-- AbsurdNumberQ(z) || SqrtNumberSumQ(z))) <-- IntegerQ(m) && FractionQ(n) && SqrtNumberSumQ(u) && SqrtNumberSumQ(v) && LtQ(u,0) && GtQ(v,0))


FixSimplify(w_'*u_^m_'*v_^n_) :=
  (With([z=Simplify((-u)^(m/GCD(m,-n))*v^(n/GCD(m,-n)))],
  (FixSimplify(-w*z^GCD(m,-n)) <-- AbsurdNumberQ(z) || SqrtNumberSumQ(z))) <-- IntegerQ(m) && FractionQ(n) && SqrtNumberSumQ(u) && SqrtNumberSumQ(1/v) && LtQ(u,0) && GtQ(v,0))
...
FixSimplify(w_'*a_^m_*(u_+b_^n_*v_')^p_') :=
  ...
  (FixSimplify(w*(a^(m/p)*u+c*v)^p) <-- RationalQ(c))) <-- RationalQ(a,b,m,n) && a>0 && b>0 && IGtQ(p,0))
...
FixSimplify(w_'*a_^m_'*(a_^n_*u_'+b_^p_'*v_')) :=
  ...
  (FixSimplify(z) <-- Not(MatchQ(z, (ww_'*aa_^mm_*(uu_+bb_^nn_*vv_') <-- RationalQ(aa,bb,mm,nn) && aa>0 && bb>0 && RationalQ(Simplify(aa^mm*bb^nn))))))) <-- RationalQ(m) && FractionQ(n) && IntegerQ(p) && p-n>0 && a+b===0)
...
FixSimplify(w_'*(a_+b_)^m_'*(c_+d_)^n_) :=
  (With([q=Simplify(b/d)],
  (FixSimplify(w*q^m*(c+d)^(m+n)) <-- FreeQ(q,Plus))) <-- IntegerQ(m) && Not(IntegerQ(n)) && EqQ(b*c-a*d,0))
...
FixSimplify(w_'*(a_^m_'*u_'+a_^n_'*v_')^t_') :=
  (FixSimplify(a^(m*t)*w*(u+a^(n-m)*v)^t) <-- Not(RationalQ(a)) && IntegerQ(t) && RationalQ(m,n) && 0<m<=n)

FixSimplify(w_'*(a_^m_'*u_'+a_^n_'*v_'+a_^p_'*z_')^t_') :=
  (FixSimplify(a^(m*t)*w*(u+a^(n-m)*v+a^(p-m)*z)^t) <-- Not(RationalQ(a)) && IntegerQ(t) && RationalQ(m,n,p) && 0<m<=n<=p)

FixSimplify(w_'*(a_^m_'*u_'+a_^n_'*v_'+a_^p_'*z_'+a_^q_'*y_')^t_') :=
  (FixSimplify(a^(m*t)*w*(u+a^(n-m)*v+a^(p-m)*z+a^(q-m)*y)^t) <-- Not(RationalQ(a)) && IntegerQ(t) && RationalQ(m,n,p) && 0<m<=n<=p<=q)
...
FixSimplify(u_'*v_^m_*w_^n_) :=
  (-FixSimplify(u*v^(m-1)) <-- RationalQ(m) && Not(RationalQ(w)) && FractionQ(n) && n<0 && EqQ(v+w^(-n),0))
...
FixSimplify(u_'*v_^m_*w_^n_') :=
  ((-1)^(n)*FixSimplify(u*v^(m+n)) <-- RationalQ(m) && Not(RationalQ(w)) && IntegerQ(n) && EqQ(v+w,0))
...
FixSimplify(u_'*(-v_^p_')^m_*w_^n_') :=
  ((-1)^(n/p)*FixSimplify(u*(-v^p)^(m+n/p)) <-- RationalQ(m) && Not(RationalQ(w)) && IntegerQ(n/p) && EqQ(v-w,0))
...
FixSimplify(u_'*(-v_^p_')^m_*w_^n_') :=
  ((-1)^(n+n/p)*FixSimplify(u*(-v^p)^(m+n/p)) <-- RationalQ(m) && Not(RationalQ(w)) && IntegersQ(n,n/p) && EqQ(v+w,0))
...
FixSimplify(u_'*(a_-b_)^m_'*(a_+b_)^m_') :=
  (u*(a^2-b^2)^m <-- IntegerQ(m) && AtomQ(a) && AtomQ(b))


FixSimplify(u_'*(c_Symbol*d_Symbol^2-e_Symbol*(b_Symbol*d_Symbol-a_Symbol*e_Symbol))^m_') :=
  (u*(c*d^2-b*d*e+a*e^2)^m <-- RationalQ(m) && OrderedQ([a,b,c,d,e]))

FixSimplify(u_'*(c_Symbol*d_Symbol^2+e_Symbol*(-b_Symbol*d_Symbol+a_Symbol*e_Symbol))^m_') :=
  (u*(c*d^2-b*d*e+a*e^2)^m <-- RationalQ(m) && OrderedQ([a,b,c,d,e]))

FixSimplify(u_'*(-c_Symbol*d_Symbol^2+e_Symbol*(b_Symbol*d_Symbol-a_Symbol*e_Symbol))^m_') :=
  ((-1)^m*u*(c*d^2-b*d*e+a*e^2)^m <-- IntegerQ(m) && OrderedQ([a,b,c,d,e]))

FixSimplify(u_'*(-c_Symbol*d_Symbol^2-e_Symbol*(-b_Symbol*d_Symbol+a_Symbol*e_Symbol))^m_') :=
  ((-1)^m*u*(c*d^2-b*d*e+a*e^2)^m <-- RationalQ(m) && OrderedQ([a,b,c,d,e]))
...
SimpFixFactor((a_'*c_^r_ + b_'*x_^n_')^p_',x_) :=
  (c^(p*r)*SimpFixFactor((a+b/c^r*x^n)^p,x) <-- FreeQ([a,b,c],x) && IntegersQ(n,p) && AtomQ(c) && RationalQ(r) && r<0)

SimpFixFactor((a_' + b_'*c_^r_*x_^n_')^p_',x_) :=
  (c^(p*r)*SimpFixFactor((a/c^r+b*x^n)^p,x) <-- FreeQ([a,b,c],x) && IntegersQ(n,p) && AtomQ(c) && RationalQ(r) && r<0)


SimpFixFactor((a_'*c_^s_' + b_'*c_^r_'*x_^n_')^p_',x_) :=
  (c^(p*s)*SimpFixFactor((a+b*c^(r-s)*x^n)^p,x) <-- FreeQ([a,b,c],x) && IntegersQ(n,p) && RationalQ(s,r) && 0<s<=r && c^(p*s)!==-1)

SimpFixFactor((a_'*c_^s_' + b_'*c_^r_'*x_^n_')^p_',x_) :=
  (c^(p*r)*SimpFixFactor((a*c^(s-r)+b*x^n)^p,x) <-- FreeQ([a,b,c],x) && IntegersQ(n,p) && RationalQ(s,r) && 0<r<s && c^(p*r)!==-1)
...
TrigSimplifyAux(u_'*(a_'*v_^m_'+b_'*v_^n_')^p_) :=
  (u*v^(m*p)*TrigSimplifyAux(a+b*v^(n-m))^p <-- InertTrigQ(v) && IntegerQ(p) && RationalQ(m,n) && m<n)
...
TrigSimplifyAux(u_'*(a_'*sec(v_)^n_'+b_'*tan(v_)^n_')^p_') :=
  (u*Sec(v)^(n*p)*(a+b*Sin(v)^n)^p <-- IntegersQ(n,p))

TrigSimplifyAux(u_'*(a_'*csc(v_)^n_'+b_'*cot(v_)^n_')^p_') :=
  (u*Csc(v)^(n*p)*(a+b*Cos(v)^n)^p <-- IntegersQ(n,p))
...
TrigSimplifyAux(u_'*(a_'*tan(v_)^n_'+b_'*sin(v_)^n_')^p_') :=
  (u*Tan(v)^(n*p)*(a+b*Cos(v)^n)^p <-- IntegersQ(n,p))

TrigSimplifyAux(u_'*(a_'*cot(v_)^n_'+b_'*cos(v_)^n_')^p_') :=
  (u*Cot(v)^(n*p)*(a+b*Sin(v)^n)^p <-- IntegersQ(n,p))
...
TrigSimplifyAux(u_'*cos(v_)^m_'*(a_'+b_'*tan(v_)^n_'+c_'*sec(v_)^n_')^p_') :=
  (u*Cos(v)^(m-n*p)*(c+b*Sin(v)^n+a*Cos(v)^n)^p <-- IntegersQ(m,n,p))

TrigSimplifyAux(u_'*sec(v_)^m_'*(a_'+b_'*tan(v_)^n_'+c_'*sec(v_)^n_')^p_') :=
  (u*Sec(v)^(m+n*p)*(c+b*Sin(v)^n+a*Cos(v)^n)^p <-- IntegersQ(m,n,p))

TrigSimplifyAux(u_'*sin(v_)^m_'*(a_'+b_'*cot(v_)^n_'+c_'*csc(v_)^n_')^p_') :=
  (u*Sin(v)^(m-n*p)*(c+b*Cos(v)^n+a*Sin(v)^n)^p <-- IntegersQ(m,n,p))

TrigSimplifyAux(u_'*csc(v_)^m_'*(a_'+b_'*cot(v_)^n_'+c_'*csc(v_)^n_')^p_') :=
  (u*Csc(v)^(m+n*p)*(c+b*Cos(v)^n+a*Sin(v)^n)^p <-- IntegersQ(m,n,p))
...
TrigSimplifyAux(u_'*(a_'*csc(v_)^m_'+b_'*sin(v_)^n_')^p_') :=
  ...
  u*((a+b*Sin(v)^(m+n))/Sin(v)^m)^p) <-- IntegersQ(m,n))

TrigSimplifyAux(u_'*(a_'*sec(v_)^m_'+b_'*cos(v_)^n_')^p_') :=
  (If(EqQ(m+n-2,0) && EqQ(a+b,0),
    u*(a*Sin(v)^2/Cos(v)^m)^p,
  u*((a+b*Cos(v)^(m+n))/Cos(v)^m)^p) <-- IntegersQ(m,n))
...
IntPart(m_*u_,n_:1) :=
  (IntPart(u,m*n) <-- RationalQ(m))

IntPart(u_,n_:1) :=
  If(RationalQ(u),
    IntegerPart(n*u),
  If(SumQ(u),
    Map(Function(IntPart(Slot(1),n)),u),
  0))
...
FracPart(m_*u_,n_:1) :=
  (FracPart(u,m*n) <-- RationalQ(m))

FracPart(u_,n_:1) :=
  If(RationalQ(u),
    FractionalPart(n*u),
  If(SumQ(u),
    Map(Function(FracPart(Slot(1),n)),u),
  n*u))
...
ContentFactorAux(expn_) :=
  ...
  If(ProductQ(expn),
    Module([num=1,tmp],
    tmp=Map(Function(If(SumQ(Slot(1)) && NumericFactor(Slot(1)[1])<0, num=-num; ContentFactorAux(-Slot(1)), ContentFactorAux(Slot(1)))), expn);
...
CommonFactors(lst_) :=
  ...
  common=num;
  lst2=Map(Function(Slot(1)/num),lst2);
  While(true,
    lst3=Map(LeadFactor,lst1);
    ( If(Apply(SameQ,lst3),
        common=common*lst3[1];
        lst1=Map(RemainingFactors,lst1),
      If(EveryQ(Function(LogQ(Slot(1)) && IntegerQ(First(Slot(1))) && First(Slot(1))>0),lst3) &&
           EveryQ(RationalQ,lst4=Map(Function(FullSimplify(Slot(1)/First(lst3))),lst3)),
        num=Apply(GCD,lst4);
        common=common*Log((First(lst3)[1])^num);
        lst2=Map2(Function(Slot(1)*Slot(2)/num),lst2,lst4);
        lst1=Map(RemainingFactors,lst1),
      ...
      If(Apply(SameQ,Map(LeadBase,lst1)) && EveryQ(RationalQ,lst4),
        ...
        lst2=Map2(Function(Slot(1)*base^(Slot(2)-num)),lst2,lst4);
        lst1=Map(RemainingFactors,lst1),
    ...
    If(EveryQ(Function(Slot(1)===1),lst1),
...
Smallest(lst_List) :=
  Module([num=lst[1]],
  Scan(Function(num=Smallest(num,Slot(1))),Rest(lst));
  num)
...
MonomialFactor(u_,x_Symbol) :=
  ...
  If(SumQ(u),
    Module([lst,deg],
    lst=Map(Function(MonomialFactor(Slot(1),x)),Apply(List,u));
    deg=lst[1,1];
    Scan(Function(deg=MinimumDegree(deg,Slot(1)[1])),Rest(lst));
    If(EqQ(deg,0) || RationalQ(deg) && deg<0,
      [0,u],
    [deg,Apply(Plus,Map(Function(x^(Slot(1)[1]-deg)*Slot(1)[2]),lst))])),
  [0,u]))))
...
ConstantFactor(u_,x_Symbol) :=
  ...
  If(ProductQ(u),
    With([lst=Map(Function(ConstantFactor(Slot(1),x)),Apply(List,u))],
    [Apply(Times,Map(First,lst)),Apply(Times,Map(Function(Slot(1)[2]),lst))]),
  If(SumQ(u),
    With([lst1=Map(Function(ConstantFactor(Slot(1),x)),Apply(List,u))],
    If(Apply(SameQ,Map(Function(Slot(1)[2]),lst1)),
      [Apply(Plus,Map(First,lst1)),lst1[1,2]],
    With([lst2=CommonFactors(Map(First,lst1))],
    [First(lst2),Apply(Plus,Map2(Times,Rest(lst2),Map(Function(Slot(1)[2]),lst1)))]))),
  [1,u])))))
...
PolynomialInAuxQ(u_,v_,x_) :=
  ...
  If(SumQ(u) || ProductQ(u),
    Catch(Scan(Function(If(Not(PolynomialInAuxQ(Slot(1),v,x)),Throw(false))),u);true),
  false))))
...
ExponentInAux(u_,v_,x_) :=
  ...
  If(ProductQ(u),
    Apply(Plus,Map(Function(ExponentInAux(Slot(1),v,x)),Apply(List,u))),
  Apply(Max,Map(Function(ExponentInAux(Slot(1),v,x)),Apply(List,u)))))))
...
PolynomialInSubst(u_,v_,x_Symbol) :=
  ...
  ReplaceAll(PolynomialInSubstAux(u,NonfreeFactors(w,x),x),[Rule(x, (x-FreeTerms(v,x))/FreeFactors(w,x))]))
...
PolynomialInSubstAux(u_,v_,x_) :=
  ...
  Map(Function(PolynomialInSubstAux(Slot(1),v,x)),u))))
...
PolynomialDivide(u_,v_,x_Symbol) :=
  ...
  quo=Apply(Plus,Map(Function(Simp(Together(Coefficient(quo,x,Slot(1))*x^Slot(1)),x)),Exponent(quo,x,List)));
  rem=Together(rem);
  free=FreeFactors(rem,x);
  rem=NonfreeFactors(rem,x);
  monomial=x^Exponent(rem,x,Min);
  If(NegQ(Coefficient(rem,x,0)), monomial=-monomial);
  rem=Apply(Plus,Map(Function(Simp(Together(Coefficient(rem,x,Slot(1))*x^Slot(1)/monomial),x)),Exponent(rem,x,List)));
#=rem=Simplify[rem]; =#
  If(BinomialQ(v,x),
...
PolynomialDivide(u_,v_,w_,x_Symbol) :=
  ReplaceAll(PolynomialDivide(PolynomialInSubst(u,w,x),PolynomialInSubst(v,w,x),x),[Rule(x, w)])
...
ExpandToSum(u_,v_,x_Symbol) :=
  ...
  r=NonfreeTerms(w,x);
  If(SumQ(r),
    u*FreeTerms(w,x)+Map(Function(MergeMonomials(u*Slot(1),x)),r),
...
ExpandToSum(u_,x_Symbol) :=
  If(PolyQ(u,x),
    Simp(Apply(Plus,Map(Function(Coeff(u,x,Slot(1))*x^Slot(1)), Expon(u,x,List))),x),
  If(BinomialQ(u,x),
    Function(Slot(1)[1] + Slot(1)[2]*x^Slot(1)[3])(BinomialParts(u,x)),
  If(TrinomialQ(u,x),
    Function(Slot(1)[1] + Slot(1)[2]*x^Slot(1)[4] + Slot(1)[3]*x^(2*Slot(1)[4]))(TrinomialParts(u,x)),
  If(GeneralizedBinomialQ(u,x),
    Function(Slot(1)[1]*x^Slot(1)[4] + Slot(1)[2]*x^Slot(1)[3])(GeneralizedBinomialParts(u,x)),
  If(GeneralizedTrinomialQ(u,x),
    Function(Slot(1)[1]*x^Slot(1)[5] + Slot(1)[2]*x^Slot(1)[4] + Slot(1)[3]*x^(2*Slot(1)[4]-Slot(1)[5]))(GeneralizedTrinomialParts(u,x)),
...
ExpandIntegrand((a_'+b_'*Log(c_'*x_^n_'))^p_',x_^m_'*(d_+e_'*x_^r_')^q_',x_Symbol) :=
  (DistributeOverTerms((a+b*Log(c*x^n))^p,CollectRecipTerms(ExpandIntegrand(x^m*(d+e*x^r)^q,x),x),x) <-- FreeQ([a,b,c,d,e,n],x) && IGtQ(p,0) && IGtQ(r,0) && ILtQ(m,0) && ILtQ(q,0)   && EqQ(r,1))
...
DistributeOverTerms(u_,v_,x_Symbol) :=
  ...
  u*FreeTerms(v,x) + If(SumQ(w), 
    Map(Function(MergeMonomials(u*Slot(1),x)),w), 
    MergeMonomials(u*w,x)))


CollectRecipTerms(u_'+A_'/x_+B_'/(d_+e_'*x_),x_Symbol) :=
  (u+A*d/(x*(d+e*x)) <-- FreeQ([A,B,d,e],x) && EqQ(B+A*e,0))
...
ExpandIntegrand(u_^p_',x_Symbol) :=
  (If(EqQ(p,1),
    ExpandCleanup(u,x),
  ExpandCleanup(Expand(u^p,x),x)) <-- SumQ(u) && IGtQ(p,0))


ExpandIntegrand((a_+b_'*x_^n_)^p_',x_Symbol) :=
  (ExpandIntegrand(x^(n*p)*(b+a*x^(-n))^p,x) <-- IntegerQ(p) && ILtQ(n,0))

ExpandIntegrand(x_^m_'*(a_+b_'*x_^n_)^p_',x_Symbol) :=
  (ExpandIntegrand(x^(m+n*p)*(b+a*x^(-n))^p,x) <-- IntegerQ(p) && ILtQ(n,0))


ExpandIntegrand(Px_'*x_^m_*(b_'*x_^n_'+c_'*x_^n1_)^p_',x_Symbol) :=
  (ExpandIntegrand(Px*x^(m+n*p)*(b+c*x)^p,x) <-- FreeQ([b,c,m],x) && PolyQ(Px,x) && IGtQ(n,0) && EqQ(n1,n+1) && IntegerQ(p))

ExpandIntegrand(Px_'*(b_'*x_^n_'+c_'*x_^n1_)^p_',x_Symbol) :=
  (ExpandIntegrand(Px*x^(n*p)*(b+c*x)^p,x) <-- FreeQ([b,c],x) && PolyQ(Px,x) && IGtQ(n,0) && EqQ(n1,n+1) && IntegerQ(p))


ExpandIntegrand((a_'+b_'*F_^u_)^p_'*(c_'+d_'*F_^v_)^q_',x_Symbol) :=
  (With([k=Simplify(u/v)],
  (ReplaceAll(ExpandIntegrand((a+b*x^Numerator(k))^p*(c+d*x^Denominator(k))^q,x),Rule(x, F^(v/Denominator(k)))) <-- RationalQ(k))) <-- FreeQ([F,a,b,c,d],x) && IntegersQ(p,q))
...
ExpandIntegrand((a_'+b_'*x_)^m_'*F_^(e_'*(c_'+d_'*x_)^n_')/(g_'+h_'*x_),x_Symbol) :=
  ...
  SimplifyTerm(tmp^m/h^m,x)*F^(e*(c+d*x)^n)/(g+h*x) + 
	Sum(SimplifyTerm(b*tmp^(k-1)/h^k,x)*F^(e*(c+d*x)^n)*(a+b*x)^(m-k),[k,1,m]))) <-- FreeQ([F,a,b,c,d,e,g,h],x) && IGtQ(m,0) && EqQ(b*c-a*d,0))
...
ExpandIntegrand(x_^m_'*(e_+f_'*x_)^p_'*F_^(b_'*(c_'+d_'*x_)^n_'),x_Symbol) :=
  ...
  ExpandIntegrand(F^(b*(c+d*x)^n),x^m*(e+f*x)^p,x))) <-- FreeQ([F,b,c,d,e,f,m,n,p],x))
...
ExpandIntegrand(x_^m_'*(e_+f_'*x_)^p_'*F_^(a_'+b_'*(c_'+d_'*x_)^n_'),x_Symbol) :=
  ...
  ExpandIntegrand(F^(a+b*(c+d*x)^n),x^m*(e+f*x)^p,x))) <-- FreeQ([F,a,b,c,d,e,f,m,n,p],x))


ExpandIntegrand(u_'*(a_+b_'*F_^v_)^m_'*(c_+d_'*F_^v_)^n_,x_Symbol) :=
  (With([w=ReplaceAll(ExpandIntegrand((a+b*x)^m*(c+d*x)^n,x),Rule(x, F^v))],
  (Map(Function(u*Slot(1)),w) <-- SumQ(w))) <-- FreeQ([F,a,b,c,d],x) && IntegersQ(m,n) && n<0)


ExpandIntegrand(u_*(a_'+b_'*x_)^m_'*F_^(e_'*(c_'+d_'*x_)^n_'),x_Symbol) :=
  (With([v=ExpandIntegrand(u*(a+b*x)^m,x)],
  (Distribute(F^(e*(c+d*x)^n)*v,Plus,Times) <-- SumQ(v))) <-- FreeQ([F,a,b,c,d,e,m,n],x) && PolynomialQ(u,x))


ExpandIntegrand(u_*(a_'+b_'*x_)^m_'*Log(c_'*(d_'+e_'*x_^n_')^p_'),x_Symbol) :=
  (ExpandIntegrand(Log(c*(d+e*x^n)^p),u*(a+b*x)^m,x) <-- FreeQ([a,b,c,d,e,m,n,p],x) && PolynomialQ(u,x))


ExpandIntegrand(u_*F_^(e_'*(c_'+d_'*x_)^n_'),x_Symbol) :=
  (If(EqQ(n,1),
    ExpandIntegrand(F^(e*(c+d*x)^n),u,x),
  ExpandLinearProduct(F^(e*(c+d*x)^n),u,c,d,x)) <-- FreeQ([F,c,d,e,n],x) && PolynomialQ(u,x))


ExpandIntegrand(F_(u_)^m_'*(a_+b_'*G_(u_))^n_',x_Symbol) :=
  (ReplaceAll(ExpandIntegrand((a+b*x)^n/x^m,x),Rule(x, G(u))) <-- FreeQ([a,b],x) && IntegersQ(m,n) && F(u)*G(u)===1)


ExpandIntegrand(u_*(a_'+b_'*Log(c_'*(d_'*(e_'+f_'*x_)^p_')^q_'))^n_,x_Symbol) :=
  (ExpandLinearProduct((a+b*Log(c*(d*(e+f*x)^p)^q))^n,u,e,f,x) <-- FreeQ([a,b,c,d,e,f,n,p,q],x) && PolynomialQ(u,x))


ExpandIntegrand(u_*(a_'+b_'*F_(c_'+d_'*x_))^n_,x_Symbol) :=
  (ExpandLinearProduct((a+b*F(c+d*x))^n,u,c,d,x) <-- FreeQ([a,b,c,d,n],x) && PolynomialQ(u,x) && MemberQ([ArcSin,ArcCos,ArcSinh,ArcCosh],F))
...
ExpandIntegrand(u_'/(a_'*x_^n_+b_'*Sqrt(c_+d_'*x_^j_)),x_Symbol) :=
  (ExpandIntegrand(u*(a*x^n-b*Sqrt(c+d*x^(2*n)))/(-b^2*c+(a^2-b^2*d)*x^(2*n)),x) <-- FreeQ([a,b,c,d,n],x) && EqQ(j,2*n))
...
ExpandIntegrand((a_+b_'*x_)^m_/(c_+d_'*x_),x_Symbol) :=
  ...
  With([tmp=a*d-b*c],
  Module([k],
  SimplifyTerm(tmp^m/d^m,x)/(c+d*x) + Sum(SimplifyTerm(b*tmp^(k-1)/d^k,x)*(a+b*x)^(m-k),[k,1,m])))) <-- FreeQ([a,b,c,d],x) && IGtQ(m,0))
...
ExpandIntegrand((a_+b_'*x_)^m_'*(A_+B_'*x_)/(c_+d_'*x_),x_Symbol) :=
  ...
  tmp2=If(SumQ(tmp2), Map(Function(SimplifyTerm(tmp1*Slot(1),x)),tmp2), SimplifyTerm(tmp1*tmp2,x));
  SimplifyTerm(B/d,x)*(a+b*x)^m + tmp2)) <-- FreeQ([a,b,c,d,A,B],x) && IGtQ(m,0))


ExpandIntegrand(u_*(a_+b_'*x_)^m_'*(c_+d_'*x_)^n_',x_Symbol) :=
  (ExpandIntegrand((c+d*x)^n,u*(a+b*x)^m,x) <-- FreeQ([a,b,c,d,m,n],x) && PolynomialQ(u,x) && Not(IntegerQ(m)) && IGtQ(n-m,0))
...
ExpandIntegrand(u_*(a_+b_'*x_)^m_',x_Symbol) :=
  ...
  If(SumQ(sum2),
    ...
    If(LeafCount(sum2)<=LeafCount(sum1)+2,
      sum2,
    sum1)),
  sum1)))) <-- FreeQ([a,b,m],x) && PolynomialQ(u,x) && 
  Not(IGtQ(m,0) && MatchQ(u,(w_'*(c_+d_'*x)^p_ <-- FreeQ([c,d],x) && IntegerQ(p) && p>m))))


ExpandIntegrand(u_*v_^n_*(a_+b_'*x_)^m_,x_Symbol) :=
  (Function(ExpandIntegrand(Slot(1)[1]*(a+b*x)^FractionalPart(m),x) + ExpandIntegrand(Slot(1)[2]*v^n*(a+b*x)^m,x))(
    PolynomialQuotientRemainder(u,v^(-n)*(a+b*x)^(-IntegerPart(m)),x)) <-- FreeQ([a,b,m],x) && ILtQ(n,0) && Not(IntegerQ(m)) && PolynomialQ(u,x) && PolynomialQ(v,x) && 
  RationalQ(m) && m<-1 && Exponent(u,x)>=-(n+IntegerPart(m))*Exponent(v,x))


ExpandIntegrand(u_*v_^n_*(a_+b_'*x_)^m_,x_Symbol) :=
  (Function(ExpandIntegrand(Slot(1)[1]*(a+b*x)^m,x) + ExpandIntegrand(Slot(1)[2]*v^n*(a+b*x)^m,x))(PolynomialQuotientRemainder(u,v^(-n),x)) <-- FreeQ([a,b,m],x) && ILtQ(n,0) && Not(IntegerQ(m)) && PolynomialQ(u,x) && PolynomialQ(v,x) && 
...
ExpandIntegrand(1/(a_+b_'*u_^n_),x_Symbol) :=
  ...
  r/(2*a*(r-s*u^(n/2))) + r/(2*a*(r+s*u^(n/2)))) <-- FreeQ([a,b],x) && IGtQ(n/2,0))
...
ExpandIntegrand((c_+d_'*u_^n_)/(a_+b_'*u_^n2_),x_Symbol) :=
  (With([r=Numerator(Rt(-a/b,2)),s=Denominator(Rt(-a/b,2))],
  -s*(d*r+c*s)/(2*b*r*(r-s*u^n)) + s*(d*r-c*s)/(2*b*r*(r+s*u^n))) <-- FreeQ([a,b,c,d],x) && IGtQ(n,0) && EqQ(n2,2*n))
...
ExpandIntegrand((a_+b_'*u_)^m_*(c_'+d_'*u_),x_Symbol) :=
  (d/b*(a+b*u)^(m+1) + (b*c-a*d)/b*(a+b*u)^m <-- FreeQ([a,b,c,d],x) && ILtQ(m,0))
...
ExpandIntegrand(1/(a_+b_'*u_^n_),x_Symbol) :=
  ...
  Module([k],
  Sum(r/(a*n*(r-(-1)^(2*k/n)*s*u)),[k,1,n]))) <-- FreeQ([a,b],x) && IGtQ(n,1))
...
ExpandIntegrand((c_+d_'*u_^m_')/(a_+b_'*u_^n_),x_Symbol) :=
  (With([r=Numerator(Rt(-a/b,n)),s=Denominator(Rt(-a/b,n))],
  Module([k],
  Sum((r*c+r*d*(r/s)^m*(-1)^(-2*k*m/n))/(a*n*(r-(-1)^(2*k/n)*s*u)),[k,1,n]))) <-- FreeQ([a,b,c,d],x) && IntegersQ(m,n) && 0<m<n)
...
ExpandIntegrand((c_'+d_'*u_^m_'+e_'*u_^p_)/(a_+b_'*u_^n_),x_Symbol) :=
  (With([r=Numerator(Rt(-a/b,n)),s=Denominator(Rt(-a/b,n))],
  Module([k],
  Sum((r*c+r*d*(r/s)^m*(-1)^(-2*k*m/n)+r*e*(r/s)^p*(-1)^(-2*k*p/n))/(a*n*(r-(-1)^(2*k/n)*s*u)),[k,1,n]))) <-- FreeQ([a,b,c,d,e],x) && IntegersQ(m,n,p) && 0<m<p<n)
...
ExpandIntegrand((c_'+d_'*u_^m_'+e_'*u_^p_+f_'*u_^q_)/(a_+b_'*u_^n_),x_Symbol) :=
  (With([r=Numerator(Rt(-a/b,n)),s=Denominator(Rt(-a/b,n))],
  Module([k],
  Sum((r*c+r*d*(r/s)^m*(-1)^(-2*k*m/n)+r*e*(r/s)^p*(-1)^(-2*k*p/n)+r*f*(r/s)^q*(-1)^(-2*k*q/n))/(a*n*(r-(-1)^(2*k/n)*s*u)),[k,1,n]))) <-- FreeQ([a,b,c,d,e,f],x) && IntegersQ(m,n,p,q) && 0<m<p<q<n)
...
ExpandIntegrand((a_+c_'*u_^n_)^p_,x_Symbol) :=
  (Module([q],
  ReplaceAll(ExpandIntegrand(1/c^p,(-q+c*x)^p*(q+c*x)^p,x),[Rule(q, Rt(-a*c,2)),Rule(x, u^(n/2))])) <-- FreeQ([a,c],x) && EvenQ(n) && ILtQ(p,0))


ExpandIntegrand(u_^m_'*(a_'+c_'*u_^n_)^p_,x_Symbol) :=
  (Module([q],
  ReplaceAll(ExpandIntegrand(1/c^p,x^m*(-q+c*x^(n/2))^p*(q+c*x^(n/2))^p,x),[Rule(q, Rt(-a*c,2)),Rule(x, u)])) <-- FreeQ([a,c],x) && IntegersQ(m,n/2) && ILtQ(p,0) && 0<m<n && Factorial(m))=n/2
...
ExpandIntegrand((a_+b_'*x_^n_)^p_,x_Symbol) :=
  (With([q=Rt(-a/b,n)],
  Module([ii],
  ExpandIntegrand((-b)^p,Product((q-(-1)^((2*ii)/n)*x)^p,[ii,1,n]),x))) <-- FreeQ([a,b],x) && IGtQ(n,1) && ILtQ(p,-1))
...
ExpandIntegrand((a_'+b_'*u_^n_'+c_'*u_^n2_')^p_,x_Symbol) :=
  (Module([q],
  ReplaceAll(ExpandIntegrand(1/(4^p*c^p),(b-q+2*c*x)^p*(b+q+2*c*x)^p,x),[Rule(q, Rt(b^2-4*a*c,2)),Rule(x, u^n)])) <-- FreeQ([a,b,c],x) && IntegerQ(n) && EqQ(n2,2*n) && ILtQ(p,0) && NeQ(b^2-4*a*c,0))


ExpandIntegrand(u_^m_'*(a_'+b_'*u_^n_'+c_'*u_^n2_')^p_,x_Symbol) :=
  (Module([q],
  ReplaceAll(ExpandIntegrand(1/(4^p*c^p),x^m*(b-q+2*c*x^n)^p*(b+q+2*c*x^n)^p,x),[Rule(q, Rt(b^2-4*a*c,2)),Rule(x, u)])) <-- FreeQ([a,b,c],x) && IntegersQ(m,n,n2) && EqQ(n2,2*n) && ILtQ(p,0) && 0<m<2*n && Not(m==n && p==-1) && NeQ(b^2-4*a*c,0))
...
ExpandIntegrand((c_+d_'*u_^n_')/(a_+b_'*u_^n2_'),x_Symbol) :=
  (With([q=Rt(-a/b,2)],
  -(c-d*q)/(2*b*q*(q+u^n)) - (c+d*q)/(2*b*q*(q-u^n))) <-- FreeQ([a,b,c,d,n],x) && EqQ(n2,2*n))
...
ExpandIntegrand((d_'+e_'*(f_'+g_'*u_^n_'))/(a_'+b_'*u_^n_'+c_'*u_^n2_'),x_Symbol) :=
  ...
  (e*g+r)/(b-q+2*c*u^n) + (e*g-r)/(b+q+2*c*u^n))) <-- FreeQ([a,b,c,d,e,f,g,n],x) && EqQ(n2,2*n) && NeQ(b^2-4*a*c,0))
...
ExpandIntegrand(u_/v_,x_Symbol) :=
  (PolynomialDivide(u,v,x) <-- PolynomialQ(u,x) && PolynomialQ(v,x) && Exponent(u,x)>=Exponent(v,x))


ExpandIntegrand(u_*(a_'*x_)^p_,x_Symbol) :=
  (ExpandToSum((a*x)^p,u,x) <-- Not(IntegerQ(p)) && PolynomialQ(u,x))


ExpandIntegrand(u_'*v_^p_,x_Symbol) :=
  (ExpandIntegrand(NormalizeIntegrand(v^p,x),u,x) <-- Not(IntegerQ(p)))
...
ExpandIntegrand(u_^m_'/(a_+b_'*u_^n_),x_Symbol) :=
  (ExpandBinomial(a,b,m,n,u,x) <-- FreeQ([a,b],x) && IntegersQ(m,n) && 0<m<n)
...
ExpandExpression(u_,x_Symbol) :=
  ...
  If(SumQ(v),
    w=NonrationalFunctionFactors(u,x);
    ExpandCleanup(Map(Function(Slot(1)*w),v),x),
  v=Expand(u,x);
  If(SumQ(v),
    ExpandCleanup(v,x),
  v=Expand(u);
...
ExpandCleanup(u_+v_/(a_+b_'*x_)+w_/(c_+d_'*x_),x_Symbol) :=
  (ExpandCleanup(u+(c*v+a*w)/(a*c+b*d*x^2),x) <-- FreeQ([a,b,c,d],x) && EqQ(b*c+a*d,0) && EqQ(d*v+b*w,0))


ExpandCleanup(u_,x_Symbol) :=
  Module([v=CollectReciprocals(u,x)],
  If(SumQ(v),
    v=Map(Function(SimplifyTerm(Slot(1),x)),v);
    If(SumQ(v),
      UnifySum(v,x),
    v),
  v))
...
CollectReciprocals(u_+e_/(a_+b_'*x_)+f_/(c_+d_'*x_),x_Symbol) :=
  (CollectReciprocals(u+(c*e+a*f)/(a*c+b*d*x^2),x) <-- FreeQ([a,b,c,d,e,f],x) && EqQ(b*c+a*d,0) && EqQ(d*e+b*f,0)) 

CollectReciprocals(u_+e_/(a_+b_'*x_)+f_/(c_+d_'*x_),x_Symbol) :=
  (CollectReciprocals(u+(d*e+b*f)*x/(a*c+b*d*x^2),x) <-- FreeQ([a,b,c,d,e,f],x) && EqQ(b*c+a*d,0) && EqQ(c*e+a*f,0))
...
MakeAssocList(u_,x_Symbol,alst_List:{}) :=
  ...
  If(FreeQ(u,x),
    With([tmp=Select(alst,Function(Slot(1)[2]===u),1)],
    If(tmp===[],
...
GensymSubst(u_,x_Symbol,alst_List) :=
  ...
  If(ProductQ(u) || SumQ(u),
    Map(Function(GensymSubst(Slot(1),x,alst)),u),
  If(FreeQ(u,x),
    With([tmp=Select(alst,Function(Slot(1)[2]===u),1)],
    If(tmp===[],
      u,
...
KernelSubst(u_,x_Symbol,alst_List) :=
  If(AtomQ(u),
    With([tmp=Select(alst,Function(Slot(1)[1]===u),1)],
    If(tmp===[],
      u,
    tmp[1,2])),
  If(IntegerPowerQ(u),
    ...
    tmp^u[2])),
  If(ProductQ(u) || SumQ(u),
    Map(Function(KernelSubst(Slot(1),x,alst)),u),
  u)))
...
ExpandAlgebraicFunction(u_Plus*v_,x_Symbol) :=
  (Map(Function(Slot(1)*v),u) <-- Not(FreeQ(u,x)))

ExpandAlgebraicFunction(u_Plus^n_*v_',x_Symbol) :=
  (With([w=Expand(u^n,x)],
  (Map(Function(Slot(1)*v),w) <-- SumQ(w))) <-- IGtQ(n,0) && Not(FreeQ(u,x)))
...
ExpandLinearProduct(v_,u_,a_,b_,x_Symbol) :=
  (Module([lst],
  lst=CoefficientList(ReplaceAll(u,Rule(x, (x-a)/b)),x);
  lst=Map(Function(SimplifyTerm(Slot(1),x)),lst);
  Module([ii],
  Sum(v*lst[ii]*(a+b*x)^(ii-1),[ii,1,Length(lst)]))) <-- FreeQ([a,b],x) && PolynomialQ(u,x))
...
ExpandTrigExpand(u_,F_,v_,m_,n_,x_Symbol) :=
  With([w=ReplaceAll(Expand(TrigExpand(F(n*x))^m,x),Rule(x, v))],
  If(SumQ(w),
    Map(Function(u*Slot(1)),w),
  u*w))
...
ExpandTrigReduce(u_'*F_(n_+v_')^m_',x_Symbol) :=
  (Module([nn],
  (ExpandTrigReduce(u*F(nn+v)^m,x) ← Rule(nn, n))) <-- MemberQ([Sinh,Cosh],F) && IntegerQ(m) && RationalQ(n))
...
ExpandTrigReduceAux(u_,x_Symbol) :=
  ...
  If(SumQ(v),
    Map(Function(NormalizeTrigReduce(Slot(1),x)),v),
  NormalizeTrigReduce(v,x)))


NormalizeTrigReduce(a_'*F_(u_)^n_',x_Symbol) :=
  (a*F(ExpandToSum(u,x))^n <-- FreeQ([F,a,n],x) && PolynomialQ(u,x) && Exponent(u,x)>0)
...
ExpandTrigToExp(u_,v_,x_Symbol) :=
  ...
  w=If(SumQ(w), Map(Function(SimplifyIntegrand(u*Slot(1),x)),w), SimplifyIntegrand(u*w,x));
...
TagSetDelayed(Dist, Dist(u_,v_,x_)+Dist(w_,v_,x_), If(EqQ(u+w,0),
    0,
  Dist(u+w,v,x)))
...
TagSetDelayed(Dist, w_*Dist(u_,v_,x_), (Dist(w*u,v,x) <-- w!==-1))

Dist(u_,Defer(Dist)(v_,w_,x_),x_) := 
  Dist(u*v,w,x)

Dist(u_,v_*w_,x_) := 
  (Dist(u*v,w,x) <-- DOLLARShowSteps===true && IntegralFreeQ(v) && Not(IntegralFreeQ(w)))
...
Dist(u_,v_,x_) := 
  ...
  If(NumericFactor(u)<0 && NumericFactor(-u)>0,
    -Dist(-u,v,x),
  If(SumQ(v),
    Map(Function(Dist(u,Slot(1),x)),v),
  If(IntegralFreeQ(v),
#=  Simp[Simp[u,x]*v,x], =#
    Simp(u*v,x),
#=If[$ShowSteps=!=True,
...
DistributeDegree(u_,m_) :=
  ...
  If(PowerQ(u),
    u[1]^(u[2]*m),
  If(ProductQ(u),
    Map(Function(DistributeDegree(Slot(1),m)),u),
  u^m)))
...
FunctionOfLinear(u_,a_,b_,x_,flag_) :=
  ...
  If(ProductQ(u) && NeQ((lst=MonomialFactor(u,x))[1],0),
    If(false && IntegerQ(lst[1]) && Factorial(lst[1])=-1 && FreeQ(lst[2],x),
  ...
  lst=[a,b];
  Catch(
  Scan(Function(lst=FunctionOfLinear(Slot(1),lst[1],lst[2],x,SumQ(u));
...
FunctionOfLinearSubst(u_,a_,b_,x_) :=
  ...
  If(ProductQ(u) && NeQ((lst=MonomialFactor(u,x))[1],0),
    ...
    FunctionOfLinearSubst(DivideDegreesOfFactors(lst[2],lst[1])*x,a,b,x)^lst[1]),
  Map(Function(FunctionOfLinearSubst(Slot(1),a,b,x)),u))))))
...
DivideDegreesOfFactors(u_,n_) :=
  If(ProductQ(u),
    Map(Function(LeadBase(Slot(1))^(LeadDegree(Slot(1))/n)),u),
...
FunctionOfInverseLinear(u_,lst_,x_) :=
  ...
  Scan(Function(If(AtomQ(tmp=FunctionOfInverseLinear(Slot(1),tmp,x)),Throw(false))),u);
  tmp))))))
...
FunctionOfExponentialFunctionAux(u_,x_) :=
  ...
  If(PowerQ(u) && FreeQ(u[1],x) && LinearQ(u[2],x),
    If(EqQ(Coefficient(DOLLARexponDOLLAR,x,0),0),
      u[1]^Coefficient(u[2],x,0)*x^FullSimplify(Log(u[1])*Coefficient(u[2],x,1)/(Log(DOLLARbaseDOLLAR)*Coefficient(DOLLARexponDOLLAR,x,1))),
    x^FullSimplify(Log(u[1])*Coefficient(u[2],x,1)/(Log(DOLLARbaseDOLLAR)*Coefficient(DOLLARexponDOLLAR,x,1)))),
  If(HyperbolicQ(u) && LinearQ(u[1],x),
    Module([tmp],
    tmp=x^FullSimplify(Coefficient(u[1],x,1)/(Log(DOLLARbaseDOLLAR)*Coefficient(DOLLARexponDOLLAR,x,1)));
    Switch(Head(u), 
      Sinh, tmp/2-1/(2*tmp),
      Cosh, tmp/2+1/(2*tmp),
  ...
  Map(Function(FunctionOfExponentialFunctionAux(Slot(1),x)),u)))))
...
FunctionOfTrig(u_,v_,x_) :=
  ...
  If(CalculusQ(u),
    false,
  Module([w=v],Catch(
  Scan(Function(If(FalseQ(w=FunctionOfTrig(Slot(1),w,x)),Throw(false))),u);
  w))))))
...
AlgebraicTrigFunctionQ(u_,x_Symbol) :=
  ...
  If(ProductQ(u) || SumQ(u),
    Catch(Scan(Function(If(Not(AlgebraicTrigFunctionQ(Slot(1),x)),Throw(false))),u); true),
  false)))))
...
FunctionOfHyperbolic(u_,v_,x_) :=
  ...
  If(CalculusQ(u),
    false,
  Module([w=v],Catch(
  Scan(Function(If(FalseQ(w=FunctionOfHyperbolic(Slot(1),w,x)),Throw(false))),u);
  w)))))
...
FunctionOfExpnQ(u_,v_,x_) :=
  ...
  If(ProductQ(u) && ProductQ(v),
    ...
    If(deg1===deg2 && FreeQ(Simplify(u/v^deg1),x),
      deg1,
    false))),
  With([lst=Map(Function(FunctionOfExpnQ(Slot(1),v,x)),Apply(List,u))],
...
PureFunctionOfSinQ(u_,v_,x_) :=
  ...
  Catch(Scan(Function(If(Not(PureFunctionOfSinQ(Slot(1),v,x)),Throw(false))),u);true))))
...
PureFunctionOfCosQ(u_,v_,x_) :=
  ...
  Catch(Scan(Function(If(Not(PureFunctionOfCosQ(Slot(1),v,x)),Throw(false))),u);true))))
...
PureFunctionOfTanQ(u_,v_,x_) :=
  ...
  Catch(Scan(Function(If(Not(PureFunctionOfTanQ(Slot(1),v,x)),Throw(false))),u);true))))
...
PureFunctionOfCotQ(u_,v_,x_) :=
  ...
  If(TrigQ(u) && EqQ(u[1],v),
    Head(u)===Cot,
  Catch(Scan(Function(If(Not(PureFunctionOfCotQ(Slot(1),v,x)),Throw(false))),u);true))))
...
#= Basis: If m integer and n odd, Tan[m*v]^n == Cos[v]*u where u is a function of Sin[v]. =#
      FunctionOfSinQ(Cos(v)*lst[2],v,x),
    Catch(Scan(Function(If(Not(FunctionOfSinQ(Slot(1),v,x)),Throw(false))),u);true)))))),
  Catch(Scan(Function(If(Not(FunctionOfSinQ(Slot(1),v,x)),Throw(false))),u);true))))))
...
#= Basis: If m integer and n odd, Tan[m*v]^n == Sin[v]*u where u is a function of Cos[v]. =#
      FunctionOfCosQ(Sin(v)*lst[2],v,x),
    Catch(Scan(Function(If(Not(FunctionOfCosQ(Slot(1),v,x)),Throw(false))),u);true)))),
  Catch(Scan(Function(If(Not(FunctionOfCosQ(Slot(1),v,x)),Throw(false))),u);true))))))
...
FunctionOfTanQ(u_,v_,x_) :=
  ...
  If(ProductQ(u),
    Module([lst=ReapList(Scan(Function(If(Not(FunctionOfTanQ(Slot(1),v,x)),Sow(Slot(1)))),u))],
    If(lst===[],
      true,
    Length(lst)==2 && OddTrigPowerQ(lst[1],v,x) && OddTrigPowerQ(lst[2],v,x))),
  Catch(Scan(Function(If(Not(FunctionOfTanQ(Slot(1),v,x)),Throw(false))),u);true)))))))
...
OddTrigPowerQ(u_,v_,x_) :=
  ...
  If(SumQ(u),
    Catch(Scan(Function(If(Not(OddTrigPowerQ(Slot(1),v,x)),Throw(false))),u);true),
  false))))
...
FunctionOfTanWeight(u_,v_,x_) :=
  ...
  If(PowerQ(u) && EvenQ(u[2]) && TrigQ(u[1]) && IntegerQuotientQ(u[1,1],v),
    ...
    -1),
  If(ProductQ(u),
    If(Catch(Scan(Function(If(Not(FunctionOfTanQ(Slot(1),v,x)),Throw(false))),u);true),
      Apply(Plus,Map(Function(FunctionOfTanWeight(Slot(1),v,x)),Apply(List,u))),
    0),
  Apply(Plus,Map(Function(FunctionOfTanWeight(Slot(1),v,x)),Apply(List,u))))))))
...
FunctionOfTrigQ(u_,v_,x_Symbol) :=
  ...
  If(TrigQ(u) && IntegerQuotientQ(u[1],v),
    true,
  Catch(
    Scan(Function(If(Not(FunctionOfTrigQ(Slot(1),v,x)),Throw(false))),u);
    true))))
...
PureFunctionOfSinhQ(u_,v_,x_) :=
  ...
  Catch(Scan(Function(If(Not(PureFunctionOfSinhQ(Slot(1),v,x)),Throw(false))),u);true))))
...
PureFunctionOfCoshQ(u_,v_,x_) :=
  ...
  Catch(Scan(Function(If(Not(PureFunctionOfCoshQ(Slot(1),v,x)),Throw(false))),u);true))))
...
PureFunctionOfTanhQ(u_,v_,x_) :=
  ...
  Catch(Scan(Function(If(Not(PureFunctionOfTanhQ(Slot(1),v,x)),Throw(false))),u);true))))
...
PureFunctionOfCothQ(u_,v_,x_) :=
  ...
  If(HyperbolicQ(u) && EqQ(u[1],v),
    Head(u)===Coth,
  Catch(Scan(Function(If(Not(PureFunctionOfCothQ(Slot(1),v,x)),Throw(false))),u);true))))
...
#= Basis: If m integer and n odd, Tanh[m*v]^n == Cosh[v]*u where u is a function of Sinh[v]. =#
      FunctionOfSinhQ(Cosh(v)*lst[2],v,x),
    Catch(Scan(Function(If(Not(FunctionOfSinhQ(Slot(1),v,x)),Throw(false))),u);true)))))),
  Catch(Scan(Function(If(Not(FunctionOfSinhQ(Slot(1),v,x)),Throw(false))),u);true))))))
...
#= Basis: If m integer and n odd, Tanh[m*v]^n == Sinh[v]*u where u is a function of Cosh[v]. =#
      FunctionOfCoshQ(Sinh(v)*lst[2],v,x),
    Catch(Scan(Function(If(Not(FunctionOfCoshQ(Slot(1),v,x)),Throw(false))),u);true)))),
  Catch(Scan(Function(If(Not(FunctionOfCoshQ(Slot(1),v,x)),Throw(false))),u);true))))))
...
FunctionOfTanhQ(u_,v_,x_) :=
  ...
  If(ProductQ(u),
    With([lst=ReapList(Scan(Function(If(Not(FunctionOfTanhQ(Slot(1),v,x)),Sow(Slot(1)))),u))],
    If(lst===[],
      true,
    Length(lst)==2 && OddHyperbolicPowerQ(lst[1],v,x) && OddHyperbolicPowerQ(lst[2],v,x))),
  Catch(Scan(Function(If(Not(FunctionOfTanhQ(Slot(1),v,x)),Throw(false))),u);true)))))))
...
OddHyperbolicPowerQ(u_,v_,x_) :=
  ...
  If(SumQ(u),
    Catch(Scan(Function(If(Not(OddHyperbolicPowerQ(Slot(1),v,x)),Throw(false))),u);true),
  false))))
...
FunctionOfTanhWeight(u_,v_,x_) :=
  ...
  If(PowerQ(u) && EvenQ(u[2]) && HyperbolicQ(u[1]) && IntegerQuotientQ(u[1,1],v),
    ...
    -1),
  If(ProductQ(u),
    If(Catch(Scan(Function(If(Not(FunctionOfTanhQ(Slot(1),v,x)),Throw(false))),u);true),
      Apply(Plus,Map(Function(FunctionOfTanhWeight(Slot(1),v,x)),Apply(List,u))),
    0),
  Apply(Plus,Map(Function(FunctionOfTanhWeight(Slot(1),v,x)),Apply(List,u))))))))
...
FunctionOfHyperbolicQ(u_,v_,x_Symbol) :=
  ...
  If(HyperbolicQ(u) && IntegerQuotientQ(u[1],v),
    true,
  Catch(Scan(Function(If(FunctionOfHyperbolicQ(Slot(1),v,x),nothing,Throw(false))),u);true))))
...
FunctionOfDensePolynomialsQ(u_,x_Symbol) :=
  ...
  Catch(
  Scan(Function(If(FunctionOfDensePolynomialsQ(Slot(1),x),nothing,Throw(false))),u);
  true)))
...
FunctionOfLog(u_,v_,n_,x_) :=
  ...
  lst=[0,v,n];
  Catch(
    [Map(Function(lst=FunctionOfLog(Slot(1),lst[2],lst[3],x);
...
PowerVariableDegree(u_,m_,c_,x_Symbol) :=
  ...
  Scan(Function(lst=PowerVariableDegree(Slot(1),lst[1],lst[2],x);If(AtomQ(lst),Throw(false))),u);
  lst)))))
...
PowerVariableSubst(u_,m_,x_Symbol) :=
  ...
  If(PowerQ(u) && FreeQ(u[1]/x,x),
    x^(u[2]/m),
  Map(Function(PowerVariableSubst(Slot(1),m,x)),u)))
...
EulerIntegrandQ((a_'*x_+b_'*u_^n_)^p_,x_Symbol) :=
  (true <-- FreeQ([a,b],x) && IntegerQ(n+1/2) && QuadraticQ(u,x) && (Not(RationalQ(p)) || ILtQ(p,0) && Not(BinomialQ(u,x))))


EulerIntegrandQ(v_^m_'*(a_'*x_+b_'*u_^n_)^p_,x_Symbol) :=
  (true <-- FreeQ([a,b],x) && EqQ(u,v) && IntegersQ(2*m,n+1/2) && QuadraticQ(u,x) && 
...
EulerIntegrandQ(u_^n_*v_^p_,x_Symbol) :=
  (true <-- ILtQ(p,0) && IntegerQ(n+1/2) && QuadraticQ(u,x) && QuadraticQ(v,x) && Not(BinomialQ(v,x)))
...
FunctionOfSquareRootOfQuadratic(u_,v_,x_Symbol) :=
  ...
  If(ProductQ(u) || SumQ(u),
    Catch(Module([lst=[v]],
    Scan(Function(lst=FunctionOfSquareRootOfQuadratic(Slot(1),lst[1],x);If(AtomQ(lst),Throw(false))),u);
    lst)),
  false)))
...
SquareRootOfQuadraticSubst(u_,vv_,xx_,x_Symbol) :=
  ...
  Map(Function(SquareRootOfQuadraticSubst(Slot(1),vv,xx,x)),u)))
...
Subst(u_,x_Symbol,v_) :=
  ...
  If(SumQ(u),
    If(BinomialQ(v,x),
      SimplifyAntiderivative(Map(Function(SubstAux(Slot(1),x,v,true)),u),x),
    SimplifyAntiderivative(Map(Function(SubstAux(Slot(1),x,v,false)),u),x)),
...
Subst(u_,(a_'*x_)^n_,v_) := 
  (If(AtomQ(u),
    u,
  If(RationalQ(n) && Factorial(Numerator(n))=1,
    Subst(u,(a*x)^(1/Denominator(n)),v/(a*x)^(n-1/Denominator(n))),
  If(PowerQ(u) && FreeQ(u[2],x) && u[1]===a*x,
    If(IntegerQ(u[2]/n),
      Simplify(v^(u[2]/n)),
    If(SumQ(u[2]),
      Apply(Times,Map(Function(Subst(u[1]^Slot(1),(a*x)^n,v)),Apply(List,u[2]))),
    With([w=Expand(u[2])],
    If(SumQ(w),
      Apply(Times,Map(Function(Subst(u[1]^Slot(1),(a*x)^n,v)),Apply(List,w))),
    With([m=NumericFactor(u[2])],
    If(Factorial(Numerator(m))=1,
  ...
  Map(Function(Subst(Slot(1),(a*x)^n,v)),u))))) <-- FreeQ([a,n],x))
...
Subst(u_,v_,w_) := 
  ...
  If(CalculusQ(u) && Not(FreeQ(v,u[2])) || HeldFormQ(u) && Head(u)!==Defer(AppellF1),
    Defer(Subst)(u,v,w),
  Map(Function(Subst(Slot(1),v,w)),u))))
...
SubstAux(a_+b_'*x_,x_,c_'*F_(z_)^2,false) :=
  (a*Simplify(1-F(z)^2) <-- FreeQ([a,b,c],x) && MemberQ([Sin,Cos,Sec,Csc,Cosh,Tanh,Coth,Sech],F) && EqQ(a+b*c,0))

SubstAux(a_+b_'*x_,x_,c_'*F_(z_)^2,false) :=
  (a*Simplify(1+F(z)^2) <-- FreeQ([a,b,c],x) && MemberQ([Tan,Cot,Sinh,Csch],F) && EqQ(a-b*c,0))


SubstAux(a_+b_'*x_^2,x_,c_'*F_(z_),false) :=
  (a*Simplify(1-F(z)^2) <-- FreeQ([a,b,c],x) && MemberQ([Sin,Cos,Sec,Csc,Cosh,Tanh,Coth,Sech],F) && EqQ(a+b*c^2,0))

SubstAux(a_+b_'*x_^2,x_,c_'*F_(z_),false) :=
  (a*Simplify(1+F(z)^2) <-- FreeQ([a,b,c],x) && MemberQ([Tan,Cot,Sinh,Csch],F) && EqQ(a-b*c^2,0))
...
SubstAux(F_(a_'*x_^m_'),x_,b_'*x_^n_,flag_) :=
  (Switch(F,
    ...
    ArcSinh, ArcCsch,  ArcCosh, ArcSech,  ArcTanh, ArcCoth,  ArcCoth, ArcTanh,  ArcSech, ArcCosh,  ArcCsch, ArcSinh)(x^(-m*n)/(a*b^m)) <-- FreeQ([a,b],x) && IGtQ(m,0) && ILtQ(n,0) && MemberQ([ArcSin,ArcCos,ArcTan,ArcCot,ArcSec,ArcCsc,ArcSinh,ArcCosh,ArcTanh,ArcCoth,ArcSech,ArcCsch],F))


SubstAux(e_+f_'*x_,x_,(a_'+b_'*x_)/(c_'+d_'*x_),flag_) :=
  (Together(c*e+a*f)/(c+d*x) <-- FreeQ([a,b,c,d,e,f],x) && EqQ(d*e+b*f,0))
...
SubstAux(u_,x_,v_,flag_) :=
  ...
  With([w=Map(Function(SubstAux(Slot(1),x,v,flag)),u)],
  If(PolyQ(w,x),
...
SimplifyAntiderivative(Log(c_*u_),x_Symbol) :=
  (SimplifyAntiderivative(Log(u),x) <-- FreeQ(c,x))
...
SimplifyAntiderivative(Log(u_^n_),x_Symbol) :=
  (n*SimplifyAntiderivative(Log(u),x) <-- FreeQ(n,x))
...
SimplifyAntiderivative(F_(G_(u_)),x_Symbol) :=
  (-SimplifyAntiderivative(F(1/G(u)),x) <-- MemberQ([Log,ArcTan,ArcCot],F) && MemberQ([Cot,Sec,Csc,Coth,Sech,Csch],G))
...
SimplifyAntiderivative(F_(G_(u_)),x_Symbol) :=
  (SimplifyAntiderivative(F(1/G(u)),x) <-- MemberQ([ArcTanh,ArcCoth],F) && MemberQ([Cot,Sec,Csc,Coth,Sech,Csch],G))


SimplifyAntiderivative(Log(F_(u_)),x_Symbol) :=
  (-SimplifyAntiderivative(Log(1/F(u)),x) <-- MemberQ([Cot,Sec,Csc,Coth,Sech,Csch],F))
...
SimplifyAntiderivative(Log(f_^u_),x_Symbol) :=
  (Log(f)*SimplifyAntiderivative(u,x) <-- FreeQ(f,x))
...
SimplifyAntiderivative(Log(a_+b_'*Tan(u_)),x_Symbol) :=
  (b/a*SimplifyAntiderivative(u,x) - SimplifyAntiderivative(Log(Cos(u)),x) <-- FreeQ([a,b],x) && EqQ(a^2+b^2,0))
...
SimplifyAntiderivative(Log(a_+b_'*Cot(u_)),x_Symbol) :=
  (-b/a*SimplifyAntiderivative(u,x) - SimplifyAntiderivative(Log(Sin(u)),x) <-- FreeQ([a,b],x) && EqQ(a^2+b^2,0))
...
SimplifyAntiderivative(ArcTan(a_'*Tan(u_)),x_Symbol) :=
  (RectifyTangent(u,a,1,x) <-- FreeQ(a,x) && GtQ(a^2,0) && ComplexFreeQ(u))
...
SimplifyAntiderivative(ArcCot(a_'*Tanh(u_)),x_Symbol) :=
  (-SimplifyAntiderivative(ArcTan(a*Tanh(u)),x) <-- FreeQ(a,x) && ComplexFreeQ(u))
...
SimplifyAntiderivative(ArcCot(a_'*Cot(u_)),x_Symbol) :=
  (RectifyCotangent(u,a,1,x) <-- FreeQ(a,x) && GtQ(a^2,0) && ComplexFreeQ(u))
...
SimplifyAntiderivative(ArcTan(a_'*Coth(u_)),x_Symbol) :=
  (-SimplifyAntiderivative(ArcTan(Tanh(u)/a),x) <-- FreeQ(a,x) && ComplexFreeQ(u))
...
SimplifyAntiderivative(ArcTanh(a_'*Coth(u_)),x_Symbol) :=
  (SimplifyAntiderivative(ArcTanh(Tanh(u)/a),x) <-- FreeQ(a,x) && ComplexFreeQ(u))
...
SimplifyAntiderivative(ArcTan(c_'*(a_+b_'*Tan(u_))),x_Symbol) :=
  (RectifyTangent(u,a*c,b*c,1,x) <-- FreeQ([a,b,c],x) && GtQ(a^2*c^2,0) && GtQ(b^2*c^2,0) && ComplexFreeQ(u))
...
SimplifyAntiderivative(ArcTan(c_'*(a_+b_'*Cot(u_))),x_Symbol) :=
  (RectifyCotangent(u,a*c,b*c,1,x) <-- FreeQ([a,b,c],x) && GtQ(a^2*c^2,0) && GtQ(b^2*c^2,0) && ComplexFreeQ(u))
...
SimplifyAntiderivative(ArcTan(a_'+b_'*Tan(u_)+c_'*Tan(u_)^2),x_Symbol) :=
  ...
  ArcTan(NormalizeTogether((c+(a-c-1)*Cos(u)^2+b*Cos(u)*Sin(u))/(c+(a-c+1)*Cos(u)^2+b*Cos(u)*Sin(u))))) <-- FreeQ([a,b,c],x) && ComplexFreeQ(u))

SimplifyAntiderivative(ArcTan(a_'+b_'*(d_'+e_'*Tan(u_))+c_'*(f_'+g_'*Tan(u_))^2),x_Symbol) :=
  (SimplifyAntiderivative(ArcTan(a+b*d+c*f^2+(b*e+2*c*f*g)*Tan(u)+c*g^2*Tan(u)^2),x) <-- FreeQ([a,b,c],x) && ComplexFreeQ(u))
...
SimplifyAntiderivative(ArcTan(a_'+c_'*Tan(u_)^2),x_Symbol) :=
  ...
  ArcTan(NormalizeTogether((c+(a-c-1)*Cos(u)^2)/(c+(a-c+1)*Cos(u)^2)))) <-- FreeQ([a,c],x) && ComplexFreeQ(u))

SimplifyAntiderivative(ArcTan(a_'+c_'*(f_'+g_'*Tan(u_))^2),x_Symbol) :=
  (SimplifyAntiderivative(ArcTan(a+c*f^2+(2*c*f*g)*Tan(u)+c*g^2*Tan(u)^2),x) <-- FreeQ([a,c],x) && ComplexFreeQ(u))
...
SimplifyAntiderivative(u_,x_Symbol) :=
  ...
  If(SumQ(u),
    SimplifyAntiderivativeSum(Map(Function(SimplifyAntiderivative(Slot(1),x)),u),x),
  u)))
...
SimplifyAntiderivativeSum(v_'+A_'*Log(a_+b_'*Tan(u_)^n_')+B_'*Log(Cos(u_)),x_Symbol) :=
  (SimplifyAntiderivativeSum(v,x) + A*Log(RemoveContent(a*Cos(u)^n+b*Sin(u)^n,x)) <-- FreeQ([a,b,A,B],x) && IntegerQ(n) && EqQ(n*A-B,0))


SimplifyAntiderivativeSum(v_'+A_'*Log(a_+b_'*Cot(u_)^n_')+B_'*Log(Sin(u_)),x_Symbol) :=
  (SimplifyAntiderivativeSum(v,x) + A*Log(RemoveContent(a*Sin(u)^n+b*Cos(u)^n,x)) <-- FreeQ([a,b,A,B],x) && IntegerQ(n) && EqQ(n*A-B,0))


SimplifyAntiderivativeSum(v_'+A_'*Log(a_+b_'*Tan(u_)^n_')+B_'*Log(c_+d_'*Tan(u_)^n_'),x_Symbol) :=
  (SimplifyAntiderivativeSum(v,x) + A*Log(RemoveContent(a*Cos(u)^n+b*Sin(u)^n,x)) + B*Log(RemoveContent(c*Cos(u)^n+d*Sin(u)^n,x)) <-- FreeQ([a,b,c,d,A,B],x) && IntegerQ(n) && EqQ(A+B,0))


SimplifyAntiderivativeSum(v_'+A_'*Log(a_+b_'*Cot(u_)^n_')+B_'*Log(c_+d_'*Cot(u_)^n_'),x_Symbol) :=
  (SimplifyAntiderivativeSum(v,x) + A*Log(RemoveContent(b*Cos(u)^n+a*Sin(u)^n,x)) + B*Log(RemoveContent(d*Cos(u)^n+c*Sin(u)^n,x)) <-- FreeQ([a,b,c,d,A,B],x) && IntegerQ(n) && EqQ(A+B,0))
...
SimplifyAntiderivativeSum(v_'+A_'*Log(a_+b_'*Tan(u_)^n_')+B_'*Log(c_+d_'*Tan(u_)^n_')+C_'*Log(e_+f_'*Tan(u_)^n_'),x_Symbol) :=
  (SimplifyAntiderivativeSum(v,x) + A*Log(RemoveContent(a*Cos(u)^n+b*Sin(u)^n,x)) + 
	B*Log(RemoveContent(c*Cos(u)^n+d*Sin(u)^n,x)) + C*Log(RemoveContent(e*Cos(u)^n+f*Sin(u)^n,x)) <-- FreeQ([a,b,c,d,e,f,A,B,C],x) && IntegerQ(n) && EqQ(A+B+C,0))
...
SimplifyAntiderivativeSum(v_'+A_'*Log(a_+b_'*Cot(u_)^n_')+B_'*Log(c_+d_'*Cot(u_)^n_')+C_'*Log(e_+f_'*Cot(u_)^n_'),x_Symbol) :=
  (SimplifyAntiderivativeSum(v,x) + A*Log(RemoveContent(b*Cos(u)^n+a*Sin(u)^n,x)) + 
	B*Log(RemoveContent(d*Cos(u)^n+c*Sin(u)^n,x)) + C*Log(RemoveContent(f*Cos(u)^n+e*Sin(u)^n,x)) <-- FreeQ([a,b,c,d,e,f,A,B,C],x) && IntegerQ(n) && EqQ(A+B+C,0))
...
SubstForAux(u_,v_,x_) :=
  ...
  Map(Function(SubstForAux(Slot(1),v,x)),u))))))
...
SubstForTrig(u_,sin_,cos_,v_,x_) :=
  ...
  If(TrigQ(u) && IntegerQuotientQ(u[1],v),
    ...
    Map(Function(SubstForTrig(Slot(1),sin,cos,v,x)),
			ReplaceAll(TrigExpand(Head(u)(Simplify(u[1]/v)*x)),Rule(x, v)))),
  If(ProductQ(u) && Head(u[1])===Cos && Head(u[2])===Sin && EqQ(u[1,1],v/2) && EqQ(u[2,1],v/2),
    sin/2*SubstForTrig(Drop(u,2),sin,cos,v,x),
  Map(Function(SubstForTrig(Slot(1),sin,cos,v,x)),u))))
...
FractionalPowerOfLinear(u_,n_,v_,x_) :=
  ...
  If(FractionalPowerQ(u) && LinearQ(u[1],x) && (FalseQ(v) || EqQ(u[1],v)),
    [LCM(Denominator(u[2]),n),u[1]],
  Catch(Module([lst=[n,v]],
    Scan(Function(If(AtomQ(lst=FractionalPowerOfLinear(Slot(1),lst[1],lst[2],x)),Throw(false))),u);
    lst)))))
...
InverseFunctionOfLinear(u_,x_Symbol) :=
  ...
  Module([tmp],
  Catch(
    Scan(Function(If(Not(AtomQ(tmp=InverseFunctionOfLinear(Slot(1),x))),Throw(tmp))),u);
    false))))
...
ActivateTrig(u_) :=
  ReplaceAll(u,[Rule(sin, Sin),Rule(cos, Cos),Rule(tan, Tan),Rule(cot, Cot),Rule(sec, Sec),Rule(csc, Csc)])
...
DeactivateTrig((c_'+d_'*x_)^m_'*(a_'+b_'*trig_(e_'+f_'*x_))^n_',x_) :=
  ((c+d*x)^m*(a+b*DeactivateTrig(trig(e+f*x),x))^n <-- FreeQ([a,b,c,d,e,f,m,n],x) && (TrigQ(trig) || HyperbolicQ(trig)))
...
FixInertTrigFunction(a_*u_,x_) :=
  (a*FixInertTrigFunction(u,x) <-- FreeQ(a,x))

FixInertTrigFunction(u_'*(a_*(b_+v_))^n_,x_) :=
  (FixInertTrigFunction(u*(a*b+a*v)^n,x) <-- FreeQ([a,b,n],x) && Not(FreeQ(v,x)))
...
FixInertTrigFunction(sec(v_)^m_'*(c_'*cos(w_))^n_',x_) :=
  (cos(v)^(-m)*(c*cos(w))^n <-- FreeQ([c,n],x) && IntegerQ(m))

FixInertTrigFunction(cot(v_)^m_'*(c_'*tan(w_))^n_',x_) :=
  (tan(v)^(-m)*(c*tan(w))^n <-- FreeQ([c,n],x) && IntegerQ(m))

FixInertTrigFunction(tan(v_)^m_'*(c_'*cot(w_))^n_',x_) :=
  (cot(v)^(-m)*(c*cot(w))^n <-- FreeQ([c,n],x) && IntegerQ(m))

FixInertTrigFunction(cos(v_)^m_'*(c_'*sec(w_))^n_',x_) :=
  (sec(v)^(-m)*(c*sec(w))^n <-- FreeQ([c,n],x) && IntegerQ(m))

FixInertTrigFunction(sin(v_)^m_'*(c_'*csc(w_))^n_',x_) :=
  (csc(v)^(-m)*(c*csc(w))^n <-- FreeQ([c,n],x) && IntegerQ(m))


FixInertTrigFunction(sec(v_)^m_'*(c_'*sin(w_))^n_',x_) :=
  (cos(v)^(-m)*(c*sin(w))^n <-- FreeQ([c,n],x) && IntegerQ(m))

FixInertTrigFunction(csc(v_)^m_'*(c_'*cos(w_))^n_',x_) :=
  (sin(v)^(-m)*(c*cos(w))^n <-- FreeQ([c,n],x) && IntegerQ(m))

FixInertTrigFunction(cos(v_)^m_'*(c_'*tan(w_))^n_',x_) :=
  (sec(v)^(-m)*(c*tan(w))^n <-- FreeQ([c,n],x) && IntegerQ(m))

FixInertTrigFunction(sin(v_)^m_'*(c_'*cot(w_))^n_',x_) :=
  (csc(v)^(-m)*(c*cot(w))^n <-- FreeQ([c,n],x) && IntegerQ(m))

FixInertTrigFunction(sin(v_)^m_'*(c_'*sec(w_))^n_',x_) :=
  (csc(v)^(-m)*(c*sec(w))^n <-- FreeQ([c,n],x) && IntegerQ(m))

FixInertTrigFunction(cos(v_)^m_'*(c_'*csc(w_))^n_',x_) :=
  (sec(v)^(-m)*(c*csc(w))^n <-- FreeQ([c,n],x) && IntegerQ(m))


FixInertTrigFunction(cot(v_)^m_'*(c_'*sin(w_))^n_',x_) :=
  (tan(v)^(-m)*(c*sin(w))^n <-- FreeQ([c,n],x) && IntegerQ(m))

FixInertTrigFunction(tan(v_)^m_'*(c_'*cos(w_))^n_',x_) :=
  (cot(v)^(-m)*(c*cos(w))^n <-- FreeQ([c,n],x) && IntegerQ(m))

FixInertTrigFunction(csc(v_)^m_'*(c_'*tan(w_))^n_',x_) :=
  (sin(v)^(-m)*(c*tan(w))^n <-- FreeQ([c,n],x) && IntegerQ(m))

FixInertTrigFunction(sec(v_)^m_'*(c_'*cot(w_))^n_',x_) :=
  (cos(v)^(-m)*(c*cot(w))^n <-- FreeQ([c,n],x) && IntegerQ(m))

FixInertTrigFunction(cot(v_)^m_'*(c_'*sec(w_))^n_',x_) :=
  (tan(v)^(-m)*(c*sec(w))^n <-- FreeQ([c,n],x) && IntegerQ(m))

FixInertTrigFunction(tan(v_)^m_'*(c_'*csc(w_))^n_',x_) :=
  (cot(v)^(-m)*(c*csc(w))^n <-- FreeQ([c,n],x) && IntegerQ(m))


FixInertTrigFunction(sec(v_)^m_'*sec(w_)^n_',x_) :=
  (cos(v)^(-m)*cos(w)^(-n) <-- IntegersQ(m,n))

FixInertTrigFunction(csc(v_)^m_'*csc(w_)^n_',x_) :=
  (sin(v)^(-m)*sin(w)^(-n) <-- IntegersQ(m,n))


FixInertTrigFunction(u_*tan(v_)^m_'*(a_+b_'*sin(w_))^n_',x_) :=
  (sin(v)^m/cos(v)^m*FixInertTrigFunction(u*(a+b*sin(w))^n,x) <-- FreeQ([a,b,n],x) && IntegerQ(m))

FixInertTrigFunction(u_*cot(v_)^m_'*(a_+b_'*sin(w_))^n_',x_) :=
  (cos(v)^m/sin(v)^m*FixInertTrigFunction(u*(a+b*sin(w))^n,x) <-- FreeQ([a,b,n],x) && IntegerQ(m))

FixInertTrigFunction(u_*tan(v_)^m_'*(a_+b_'*cos(w_))^n_',x_) :=
  (sin(v)^m/cos(v)^m*FixInertTrigFunction(u*(a+b*cos(w))^n,x) <-- FreeQ([a,b,n],x) && IntegerQ(m))

FixInertTrigFunction(u_*cot(v_)^m_'*(a_+b_'*cos(w_))^n_',x_) :=
  (cos(v)^m/sin(v)^m*FixInertTrigFunction(u*(a+b*cos(w))^n,x) <-- FreeQ([a,b,n],x) && IntegerQ(m))


FixInertTrigFunction(cot(v_)^m_'*(a_'+b_'*(c_'*sin(w_))^p_')^n_',x_) :=
  (tan(v)^(-m)*(a+b*(c*sin(w))^p)^n <-- FreeQ([a,b,c,n,p],x) && IntegerQ(m))

FixInertTrigFunction(tan(v_)^m_'*(a_'+b_'*(c_'*cos(w_))^p_')^n_',x_) :=
  (cot(v)^(-m)*(a+b*(c*cos(w))^p)^n <-- FreeQ([a,b,c,n,p],x) && IntegerQ(m))


FixInertTrigFunction(u_'*(c_'*sin(v_)^n_')^p_'*w_,x_) :=
  ((c*sin(v)^n)^p*FixInertTrigFunction(u*w,x) <-- FreeQ([c,p],x) && PowerOfInertTrigSumQ(w,sin,x))

FixInertTrigFunction(u_'*(c_'*cos(v_)^n_')^p_'*w_,x_) :=
  ((c*cos(v)^n)^p*FixInertTrigFunction(u*w,x) <-- FreeQ([c,p],x) && PowerOfInertTrigSumQ(w,cos,x))

FixInertTrigFunction(u_'*(c_'*tan(v_)^n_')^p_'*w_,x_) :=
  ((c*tan(v)^n)^p*FixInertTrigFunction(u*w,x) <-- FreeQ([c,p],x) && PowerOfInertTrigSumQ(w,tan,x))

FixInertTrigFunction(u_'*(c_'*cot(v_)^n_')^p_'*w_,x_) :=
  ((c*cot(v)^n)^p*FixInertTrigFunction(u*w,x) <-- FreeQ([c,p],x) && PowerOfInertTrigSumQ(w,cot,x))

FixInertTrigFunction(u_'*(c_'*sec(v_)^n_')^p_'*w_,x_) :=
  ((c*sec(v)^n)^p*FixInertTrigFunction(u*w,x) <-- FreeQ([c,p],x) && PowerOfInertTrigSumQ(w,sec,x))

FixInertTrigFunction(u_'*(c_'*csc(v_)^n_')^p_'*w_,x_) :=
  ((c*csc(v)^n)^p*FixInertTrigFunction(u*w,x) <-- FreeQ([c,p],x) && PowerOfInertTrigSumQ(w,csc,x))


FixInertTrigFunction(u_'*sec(v_)^n_'*w_,x_) :=
  (cos(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,cos,x) && IntegerQ(n))

FixInertTrigFunction(u_'*csc(v_)^n_'*w_,x_) :=
  (sin(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,sin,x) && IntegerQ(n))

FixInertTrigFunction(u_'*sec(v_)^n_'*w_,x_) :=
  (cos(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,sin,x) && IntegerQ(n))

FixInertTrigFunction(u_'*csc(v_)^n_'*w_,x_) :=
  (sin(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,cos,x) && IntegerQ(n))


FixInertTrigFunction(u_'*cot(v_)^n_'*w_,x_) :=
  (tan(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,tan,x) && IntegerQ(n))

FixInertTrigFunction(u_'*cos(v_)^n_'*w_,x_) :=
  (sec(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,tan,x) && IntegerQ(n))

FixInertTrigFunction(u_'*cos(v_)^n_*w_,x_) :=
  (sec(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,tan,x) && IntegerQ(n))

FixInertTrigFunction(u_'*csc(v_)^n_'*w_,x_) :=
  (sin(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,tan,x) && IntegerQ(n))


FixInertTrigFunction(u_'*tan(v_)^n_'*w_,x_) :=
  (cot(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,cot,x) && IntegerQ(n))

FixInertTrigFunction(u_'*sin(v_)^n_'*w_,x_) :=
  (csc(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,cot,x) && IntegerQ(n))

FixInertTrigFunction(u_'*sin(v_)^n_'*w_,x_) :=
  (csc(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,cot,x) && IntegerQ(n))

FixInertTrigFunction(u_'*sec(v_)^n_'*w_,x_) :=
  (cos(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,cot,x) && IntegerQ(n))


FixInertTrigFunction(u_'*cos(v_)^n_'*w_,x_) :=
  (sec(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,sec,x) && IntegerQ(n))

FixInertTrigFunction(u_'*cot(v_)^n_'*w_,x_) :=
  (tan(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,sec,x) && IntegerQ(n))

FixInertTrigFunction(u_'*csc(v_)^n_'*w_,x_) :=
  (sin(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,sec,x) && IntegerQ(n))


FixInertTrigFunction(u_'*sin(v_)^n_'*w_,x_) :=
  (csc(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,csc,x) && IntegerQ(n))

FixInertTrigFunction(u_'*tan(v_)^n_'*w_,x_) :=
  (cot(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,csc,x) && IntegerQ(n))

FixInertTrigFunction(u_'*sec(v_)^n_'*w_,x_) :=
  (cos(v)^(-n)*FixInertTrigFunction(u*w,x) <-- PowerOfInertTrigSumQ(w,csc,x) && IntegerQ(n))


FixInertTrigFunction(u_'*tan(v_)^m_'*(a_'*sin(v_)+b_'*cos(v_))^n_',x_) :=
  (sin(v)^m*cos(v)^(-m)*FixInertTrigFunction(u*(a*sin(v)+b*cos(v))^n,x) <-- FreeQ([a,b,n],x) && IntegerQ(m))

FixInertTrigFunction(u_'*cot(v_)^m_'*(a_'*sin(v_)+b_'*cos(v_))^n_',x_) :=
  (cos(v)^m*sin(v)^(-m)*FixInertTrigFunction(u*(a*sin(v)+b*cos(v))^n,x) <-- FreeQ([a,b,n],x) && IntegerQ(m))

FixInertTrigFunction(u_'*sec(v_)^m_'*(a_'*sin(v_)+b_'*cos(v_))^n_',x_) :=
  (cos(v)^(-m)*FixInertTrigFunction(u*(a*sin(v)+b*cos(v))^n,x) <-- FreeQ([a,b,n],x) && IntegerQ(m))

FixInertTrigFunction(u_'*csc(v_)^m_'*(a_'*sin(v_)+b_'*cos(v_))^n_',x_) :=
  (sin(v)^(-m)*FixInertTrigFunction(u*(a*sin(v)+b*cos(v))^n,x) <-- FreeQ([a,b,n],x) && IntegerQ(m))


FixInertTrigFunction(f_(v_)^m_'*(A_'+B_'*g_(v_)+C_'*g_(v_)^2),x_) :=
  (g(v)^(-m)*(A+B*g(v)+C*g(v)^2) <-- FreeQ([A,B,C],x) && IntegerQ(m) && (InertReciprocalQ(f,g) || InertReciprocalQ(g,f)))

FixInertTrigFunction(f_(v_)^m_'*(A_'+C_'*g_(v_)^2),x_) :=
  (g(v)^(-m)*(A+C*g(v)^2) <-- FreeQ([A,C],x) && IntegerQ(m) && (InertReciprocalQ(f,g) || InertReciprocalQ(g,f)))


FixInertTrigFunction(f_(v_)^m_'*(A_'+B_'*g_(v_)+C_'*g_(v_)^2)*(a_'+b_'*g_(v_))^n_',x_) :=
  (g(v)^(-m)*(A+B*g(v)+C*g(v)^2)*(a+b*g(v))^n <-- FreeQ([a,b,A,B,C,n],x) && IntegerQ(m) && (InertReciprocalQ(f,g) || InertReciprocalQ(g,f)))

FixInertTrigFunction(f_(v_)^m_'*(A_'+C_'*g_(v_)^2)*(a_'+b_'*g_(v_))^n_',x_) :=
  (g(v)^(-m)*(A+C*g(v)^2)*(a+b*g(v))^n <-- FreeQ([a,b,A,C,n],x) && IntegerQ(m) && (InertReciprocalQ(f,g) || InertReciprocalQ(g,f)))
...
UnifyInertTrigFunction(a_*u_,x_) :=
  (a*UnifyInertTrigFunction(u,x) <-- FreeQ(a,x))
...
DerivativeDivides(y_,u_,x_Symbol) :=
  ...
  If(If(PolynomialQ(y,x), PolynomialQ(u,x) && Exponent(u,x)==Exponent(y,x)-1, EasyDQ(y,x)),
    Module([v=Block([DOLLARShowSteps=false], ReplaceAll(D(y,x),Rule(Sinc(z_), Sin(z)/z)))],
    If(EqQ(v,0),
      false,
    v=Simplify(u/v);
    If(FreeQ(v,x),
...
TrigSquare(u_) :=
  If(SumQ(u),
    With([lst=SplitSum(Function(SplitProduct(TrigSquareQ,Slot(1))),u)],
...
RtAux(u_,n_) :=
  ...
  If(ProductQ(u),
    Module([lst],
    lst=SplitProduct(Function(GtQ(Slot(1),0)),u);
    If(ListQ(lst),
      RtAux(lst[1],n)*RtAux(lst[2],n),
    lst=SplitProduct(Function(LtQ(Slot(1),0)),u);
    If(ListQ(lst),
      If(EqQ(lst[1],-1),
        With([v=lst[2]],
...
IntTerm(u_,x_Symbol) :=
  (Map(Function(IntTerm(Slot(1),x)),u) <-- SumQ(u))
...
IntTerm(c_'*v_^m_',x_Symbol) :=
  ...
  If(EqQ(m,1) && EqQ(c,1) && SumQ(u),
    IntSum(u,x),
  Simp(c*u^(m+1)/(Coeff(u,x,1)*(m+1)),x)))) <-- FreeQ([c,m],x) && LinearQ(v,x))
...
FixIntRules() :=
  (DownValues(Int)=FixIntRules(DownValues(Int)); nothing)


FixIntRules(rulelist_) := Block([Int, Subst, Simp, Dist],
  SetAttributes([Simp,Dist,Int,Subst],HoldAll);
  Map(Function(FixIntRule(Slot(1),Slot(1)[1,1,2,1])), rulelist))
...
FixIntRule(RuleDelayed(lhs_,F_(G_(list_,F_(u_+v_,test2_)),test1_)),x_) :=
  (ReplacePart(RuleDelayed(lhs,F(G(list,F(u+v,test2)),test1)),[Rule([2,1,2,1,1], FixRhsIntRule(u,x)),Rule([2,1,2,1,2], FixRhsIntRule(v,x))]) <-- F===Condition && (G===With || G===Module || G===Block))

FixIntRule(RuleDelayed(lhs_,G_(list_,F_(u_+v_,test2_))),x_) :=
  (ReplacePart(RuleDelayed(lhs,G(list,F(u+v,test2))),[Rule([2,2,1,1], FixRhsIntRule(u,x)),Rule([2,2,1,2], FixRhsIntRule(v,x))]) <-- F===Condition && (G===With || G===Module || G===Block))

FixIntRule(RuleDelayed(lhs_,F_(G_(list_,u_+v_),test_)),x_) :=
  (ReplacePart(RuleDelayed(lhs,F(G(list,u+v),test)),[Rule([2,1,2,1], FixRhsIntRule(u,x)),Rule([2,1,2,2], FixRhsIntRule(v,x))]) <-- F===Condition && (G===With || G===Module || G===Block))

FixIntRule(RuleDelayed(lhs_,G_(list_,u_+v_)),x_) :=
  (ReplacePart(RuleDelayed(lhs,G(list,u+v)),[Rule([2,2,1], FixRhsIntRule(u,x)),Rule([2,2,2], FixRhsIntRule(v,x))]) <-- (G===With || G===Module || G===Block))

FixIntRule(RuleDelayed(lhs_,F_(u_+v_,test_)),x_) :=
  (ReplacePart(RuleDelayed(lhs,F(u+v,test)),[Rule([2,1,1], FixRhsIntRule(u,x)),Rule([2,1,2], FixRhsIntRule(v,x))]) <-- F===Condition)

FixIntRule(RuleDelayed(lhs_,u_+v_),x_) :=
  ReplacePart(RuleDelayed(lhs,u+v),[Rule([2,1], FixRhsIntRule(u,x)),Rule([2,2], FixRhsIntRule(v,x))])


FixIntRule(RuleDelayed(lhs_,F_(G_(list1_,F_(H_(list2_,u_),test2_)),test1_)),x_) :=
  (ReplacePart(RuleDelayed(lhs,F(G(list1,F(H(list2,u),test2)),test1)),Rule([2,1,2,1,2], FixRhsIntRule(u,x))) <-- F===Condition && (G===With || G===Module || G===Block) && (H===With || H===Module || H===Block))

FixIntRule(RuleDelayed(lhs_,F_(G_(list1_,H_(list2_,u_)),test1_)),x_) :=
  (ReplacePart(RuleDelayed(lhs,F(G(list1,H(list2,u)),test1)),Rule([2,1,2,2], FixRhsIntRule(u,x))) <-- F===Condition && (G===With || G===Module || G===Block) && (H===With || H===Module || H===Block))

FixIntRule(RuleDelayed(lhs_,F_(G_(list_,F_(H_(str1_,str2_,str3_,J_(u_)),test2_)),test1_)),x_) :=
  (ReplacePart(RuleDelayed(lhs,F(G(list,F(H(str1,str2,str3,J(u)),test2)),test1)),Rule([2,1,2,1,4,1], FixRhsIntRule(u,x))) <-- F===Condition && (G===With || G===Module || G===Block) && H===ShowStep && J===Hold)

FixIntRule(RuleDelayed(lhs_,F_(G_(list_,F_(u_,test2_)),test1_)),x_) :=
  (ReplacePart(RuleDelayed(lhs,F(G(list,F(u,test2)),test1)),Rule([2,1,2,1], FixRhsIntRule(u,x))) <-- F===Condition && (G===With || G===Module || G===Block))

FixIntRule(RuleDelayed(lhs_,G_(list_,F_(u_,test2_))),x_) :=
  (ReplacePart(RuleDelayed(lhs,G(list,F(u,test2))),Rule([2,2,1], FixRhsIntRule(u,x))) <-- F===Condition && (G===With || G===Module || G===Block))

FixIntRule(RuleDelayed(lhs_,F_(G_(list_,u_),test_)),x_) :=
  (ReplacePart(RuleDelayed(lhs,F(G(list,u),test)),Rule([2,1,2], FixRhsIntRule(u,x))) <-- F===Condition && (G===With || G===Module || G===Block))

FixIntRule(RuleDelayed(lhs_,G_(list_,u_)),x_) :=
  (ReplacePart(RuleDelayed(lhs,G(list,u)),Rule([2,2], FixRhsIntRule(u,x))) <-- (G===With || G===Module || G===Block))

FixIntRule(RuleDelayed(lhs_,F_(u_,test_)),x_) :=
  (ReplacePart(RuleDelayed(lhs,F(u,test)),Rule([2,1], FixRhsIntRule(u,x))) <-- F===Condition)

FixIntRule(RuleDelayed(lhs_,u_),x_) :=
  ReplacePart(RuleDelayed(lhs,u),Rule([2], FixRhsIntRule(u,x)))
...
FixRhsIntRule(a_*u_,x_) :=
  (Dist(a,u,x) <-- MemberQ([Int,Subst],Head(Unevaluated(u))))
