...
Int(x_^m_'*(a_' + b_'*Log(c_'*x_))^p_, x_Symbol) := (1/c^(m + 1)*Subst(Int(ℯ^((m + 1)*x)*(a + b*x)^p, x), x, Log(c*x)) <-- FreeQ([a, b, c, p], x) && IntegerQ(m))
Int((d_'*x_)^m_'*(a_' + b_'*Log(c_'*x_^n_'))^p_, x_Symbol) := ((d*x)^(m + 1)/(d*n*(c*x^n)^((m + 1)/n))* Subst(Int(ℯ^((m + 1)/n*x)*(a + b*x)^p, x), x, Log(c*x^n)) <-- FreeQ([a, b, c, d, m, n, p], x))
