...
Int((a_' + b_'*Log(c_'*x_^n_'))^p_, x_Symbol) := (1/(n*c^(1/n))*Subst(Int(ℯ^(x/n)*(a + b*x)^p, x), x, Log(c*x^n)) <-- FreeQ([a, b, c, p], x) && IntegerQ(1/n))
Int((a_' + b_'*Log(c_'*x_^n_'))^p_, x_Symbol) := (x/(n*(c*x^n)^(1/n))* Subst(Int(ℯ^(x/n)*(a + b*x)^p, x), x, Log(c*x^n)) <-- FreeQ([a, b, c, n, p], x))
end
