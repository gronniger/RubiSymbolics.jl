...
DOLLARUseGamma = false;
Int((c_' + d_'*x_)^m_'*(b_'*F_^(g_'*(e_' + f_'*x_)))^n_', x_Symbol) := ((c + d*x)^m*(b*F^(g*(e + f*x)))^n/(f*g*n*Log(F)) - d*m/(f*g*n*Log(F))* Int((c + d*x)^(m - 1)*(b*F^(g*(e + f*x)))^n, x) <-- FreeQ([F, b, c, d, e, f, g, n], x) && GtQ(m, 0) && IntegerQ(2*m) && Not(TrueQ(DOLLARUseGamma)))
Int((c_' + d_'*x_)^m_*(b_'*F_^(g_'*(e_' + f_'*x_)))^n_', x_Symbol) := ((c + d*x)^(m + 1)*(b*F^(g*(e + f*x)))^ n/(d*(m + 1)) - f*g*n*Log(F)/(d*(m + 1))* Int((c + d*x)^(m + 1)*(b*F^(g*(e + f*x)))^n, x) <-- FreeQ([F, b, c, d, e, f, g, n], x) && LtQ(m, -1) && IntegerQ(2*m) && Not(TrueQ(DOLLARUseGamma)))
Int(F_^(g_'*(e_' + f_'*x_))/(c_' + d_'*x_), x_Symbol) := (F^(g*(e - c*f/d))/d*ExpIntegralEi(f*g*(c + d*x)*Log(F)/d) <-- FreeQ([F, c, d, e, f, g], x) && Not(TrueQ(DOLLARUseGamma)))
...
Int(F_^(g_'*(e_' + f_'*x_))/Sqrt(c_' + d_'*x_), x_Symbol) := (2/d*Subst(Int(F^(g*(e - c*f/d) + f*g*x^2/d), x), x, Sqrt(c + d*x)) <-- FreeQ([F, c, d, e, f, g], x) && Not(TrueQ(DOLLARUseGamma)))
