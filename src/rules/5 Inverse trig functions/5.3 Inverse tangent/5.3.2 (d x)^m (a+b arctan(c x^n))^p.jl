...
Int((a_' + b_'*ArcTan(c_'*x_))/x_, x_Symbol) := (a*Log(x) + im*b/2*Int(Log(1 - im*c*x)/x, x) - im*b/2*Int(Log(1 + im*c*x)/x, x) <-- FreeQ([a, b, c], x))
Int((a_' + b_'*ArcCot(c_'*x_))/x_, x_Symbol) := (a*Log(x) + im*b/2*Int(Log(1 - im/(c*x))/x, x) - im*b/2*Int(Log(1 + im/(c*x))/x, x) <-- FreeQ([a, b, c], x))
Int((a_' + b_'*ArcTan(c_'*x_))^p_/x_, x_Symbol) := (2*(a + b*ArcTan(c*x))^p*ArcTanh(1 - 2/(1 + im*c*x)) - 2*b*c*p* Int((a + b*ArcTan(c*x))^(p - 1)* ArcTanh(1 - 2/(1 + im*c*x))/(1 + c^2*x^2), x) <-- FreeQ([a, b, c], x) && IGtQ(p, 1))
Int((a_' + b_'*ArcCot(c_'*x_))^p_/x_, x_Symbol) := (2*(a + b*ArcCot(c*x))^p*ArcCoth(1 - 2/(1 + im*c*x)) + 2*b*c*p* Int((a + b*ArcCot(c*x))^(p - 1)* ArcCoth(1 - 2/(1 + im*c*x))/(1 + c^2*x^2), x) <-- FreeQ([a, b, c], x) && IGtQ(p, 1))
...
Int(x_^m_'*(a_' + b_'*ArcTan(c_'*x_^n_))^p_, x_Symbol) := (Int(ExpandIntegrand( x^m*(a + (im*b*Log(1 - im*c*x^n))/2 - (im*b*Log(1 + im*c*x^n))/2)^p, x), x) <-- FreeQ([a, b, c], x) && IGtQ(p, 1) && IGtQ(n, 0) && IntegerQ(m))
Int(x_^m_'*(a_' + b_'*ArcCot(c_'*x_^n_))^p_, x_Symbol) := (Int(ExpandIntegrand( x^m*(a + (im*b*Log(1 - im*x^(-n)/c))/2 - (im*b*Log(1 + im*x^(-n)/c))/ 2)^p, x), x) <-- FreeQ([a, b, c], x) && IGtQ(p, 1) && IGtQ(n, 0) && IntegerQ(m))
