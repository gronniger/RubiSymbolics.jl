...
Int(u_^r_'*(e_'*(a_' + b_'*x_^n_')/(c_ + d_'*x_^n_'))^p_, x_Symbol) := (With([q = Denominator(p)], q*e*(b*c - a*d)/n* Subst(Int( SimplifyIntegrand( x^(q*(p + 1) - 1)*(-a*e + c*x^q)^(1/n - 1)/(b*e - d*x^q)^(1/n + 1)* ReplaceAll(u, Rule(x, (-a*e + c*x^q)^(1/n)/(b*e - d*x^q)^(1/n)))^r, x), x), x, (e*(a + b*x^n)/(c + d*x^n))^(1/q))) <-- FreeQ([a, b, c, d, e], x) && PolynomialQ(u, x) && FractionQ(p) && IntegerQ(1/n) && IntegerQ(r))
Int(x_^m_'*u_^r_'*(e_'*(a_' + b_'*x_^n_')/(c_ + d_'*x_^n_'))^p_, x_Symbol) := (With([q = Denominator(p)], q*e*(b*c - a*d)/n* Subst(Int( SimplifyIntegrand( x^(q*(p + 1) - 1)*(-a*e + c*x^q)^((m + 1)/n - 1)/(b*e - d*x^q)^((m + 1)/n + 1)* ReplaceAll(u, Rule(x, (-a*e + c*x^q)^(1/n)/(b*e - d*x^q)^(1/n)))^r, x), x), x, (e*(a + b*x^n)/(c + d*x^n))^(1/q))) <-- FreeQ([a, b, c, d, e], x) && PolynomialQ(u, x) && FractionQ(p) && IntegerQ(1/n) && IntegersQ(m, r))
