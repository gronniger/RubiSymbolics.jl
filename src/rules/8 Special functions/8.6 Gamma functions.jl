...
Int(Gamma(n_, a_' + b_'*x_)/(c_' + d_'*x_), x_Symbol) := (Int((a + b*x)^(n - 1)/((c + d*x)*ℯ^(a + b*x)), x) + (n - 1)* Int(Gamma(n - 1, a + b*x)/(c + d*x), x) <-- FreeQ([a, b, c, d], x) && IGtQ(n, 1))
Int((c_' + d_'*x_)^m_'*Gamma(n_, a_' + b_'*x_), x_Symbol) := (Block([DOLLARUseGamma = true], (c + d*x)^(m + 1)*Gamma(n, a + b*x)/(d*(m + 1)) + b/(d*(m + 1))* Int((c + d*x)^(m + 1)*(a + b*x)^(n - 1)/ℯ^(a + b*x), x)) <-- FreeQ([a, b, c, d, m, n], x) && (IGtQ(m, 0) || IGtQ(n, 0) || IntegersQ(m, n)) && NeQ(m, -1))
...
Int((Factorial((a_' + b_'*x_)))^n_'*PolyGamma(0, c_' + b_'*x_), x_Symbol) := ((Factorial((a + b*x)))^n/(b*n) <-- FreeQ([a, b, c, n], x) && EqQ(c, a + 1))
Int(Gamma(p_, d_'*(a_' + b_'*Log(c_'*x_^n_'))), x_Symbol) := (x*Gamma(p, d*(a + b*Log(c*x^n))) + b*d*n*ℯ^(-a*d)* Int((d*(a + b*Log(c*x^n)))^(p - 1)/(c*x^n)^(b*d), x) <-- FreeQ([a, b, c, d, n, p], x))
Int(Gamma(p_, d_'*(a_' + b_'*Log(c_'*x_^n_')))/x_, x_Symbol) := (1/n*Subst(Gamma(p, d*(a + b*x)), x, Log(c*x^n)) <-- FreeQ([a, b, c, d, n, p], x))
Int((e_'*x_)^m_'*Gamma(p_, d_'*(a_' + b_'*Log(c_'*x_^n_'))), x_Symbol) := ((e*x)^(m + 1)* Gamma(p, d*(a + b*Log(c*x^n)))/(e*(m + 1)) + b*d*n*ℯ^(-a*d)*(e*x)^(b*d*n)/((m + 1)*(c*x^n)^(b*d))* Int((e*x)^(m - b*d*n)*(d*(a + b*Log(c*x^n)))^(p - 1), x) <-- FreeQ([a, b, c, d, e, m, n, p], x) && NeQ(m, -1))
