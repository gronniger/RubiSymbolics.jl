...
Int(ExpIntegralEi(a_' + b_'*x_), x_Symbol) := ((a + b*x)*ExpIntegralEi(a + b*x)/b - ℯ^(a + b*x)/b <-- FreeQ([a, b], x))
...
Int((c_' + d_'*x_)^m_'*ExpIntegralEi(a_' + b_'*x_), x_Symbol) := ((c + d*x)^(m + 1)* ExpIntegralEi(a + b*x)/(d*(m + 1)) - b/(d*(m + 1))*Int((c + d*x)^(m + 1)*ℯ^(a + b*x)/(a + b*x), x) <-- FreeQ([a, b, c, d, m], x) && NeQ(m, -1))
Int(ExpIntegralEi(a_' + b_'*x_)^2, x_Symbol) := ((a + b*x)*ExpIntegralEi(a + b*x)^2/b - 2*Int(ℯ^(a + b*x)*ExpIntegralEi(a + b*x), x) <-- FreeQ([a, b], x))
Int(x_^m_'*ExpIntegralEi(b_'*x_)^2, x_Symbol) := (x^(m + 1)*ExpIntegralEi(b*x)^2/(m + 1) - 2/(m + 1)*Int(x^m*ℯ^(b*x)*ExpIntegralEi(b*x), x) <-- FreeQ(b, x) && IGtQ(m, 0))
Int(x_^m_'*ExpIntegralEi(a_ + b_'*x_)^2, x_Symbol) := (x^(m + 1)*ExpIntegralEi(a + b*x)^2/(m + 1) + a*x^m*ExpIntegralEi(a + b*x)^2/(b*(m + 1)) - 2/(m + 1)*Int(x^m*ℯ^(a + b*x)*ExpIntegralEi(a + b*x), x) - a*m/(b*(m + 1))*Int(x^(m - 1)*ExpIntegralEi(a + b*x)^2, x) <-- FreeQ([a, b], x) && IGtQ(m, 0))
...
Int(ℯ^(a_' + b_'*x_)*ExpIntegralEi(c_' + d_'*x_), x_Symbol) := (ℯ^(a + b*x)*ExpIntegralEi(c + d*x)/b - d/b*Int(ℯ^(a + c + (b + d)*x)/(c + d*x), x) <-- FreeQ([a, b, c, d], x))
Int(x_^m_'*ℯ^(a_' + b_'*x_)*ExpIntegralEi(c_' + d_'*x_), x_Symbol) := (x^m*ℯ^(a + b*x)*ExpIntegralEi(c + d*x)/b - d/b*Int(x^m*ℯ^(a + c + (b + d)*x)/(c + d*x), x) - m/b*Int(x^(m - 1)*ℯ^(a + b*x)*ExpIntegralEi(c + d*x), x) <-- FreeQ([a, b, c, d], x) && IGtQ(m, 0))
Int(x_^m_*ℯ^(a_' + b_'*x_)*ExpIntegralEi(c_' + d_'*x_), x_Symbol) := (x^(m + 1)*ℯ^(a + b*x)*ExpIntegralEi(c + d*x)/(m + 1) - d/(m + 1)*Int(x^(m + 1)*ℯ^(a + c + (b + d)*x)/(c + d*x), x) - b/(m + 1)*Int(x^(m + 1)*ℯ^(a + b*x)*ExpIntegralEi(c + d*x), x) <-- FreeQ([a, b, c, d], x) && ILtQ(m, -1))
Int(ExpIntegralEi(d_'*(a_' + b_'*Log(c_'*x_^n_'))), x_Symbol) := (x*ExpIntegralEi(d*(a + b*Log(c*x^n))) - b*n*ℯ^(a*d)*Int((c*x^n)^(b*d)/(a + b*Log(c*x^n)), x) <-- FreeQ([a, b, c, d, n], x))
Int(ExpIntegralEi(d_'*(a_' + b_'*Log(c_'*x_^n_')))/x_, x_Symbol) := (1/n*Subst(ExpIntegralEi(d*(a + b*x)), x, Log(c*x^n)) <-- FreeQ([a, b, c, d, n], x))
Int((e_'*x_)^m_'*ExpIntegralEi(d_'*(a_' + b_'*Log(c_'*x_^n_'))), x_Symbol) := ((e*x)^(m + 1)* ExpIntegralEi(d*(a + b*Log(c*x^n)))/(e*(m + 1)) - b*n*ℯ^(a*d)*(c*x^n)^(b*d)/((m + 1)*(e*x)^(b*d*n))* Int((e*x)^(m + b*d*n)/(a + b*Log(c*x^n)), x) <-- FreeQ([a, b, c, d, e, m, n], x) && NeQ(m, -1))
