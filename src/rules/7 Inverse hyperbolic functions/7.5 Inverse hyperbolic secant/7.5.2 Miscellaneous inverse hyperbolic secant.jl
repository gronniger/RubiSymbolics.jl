...
Int(ℯ^ArcSech(a_'*x_), x_Symbol) := (x*ℯ^ArcSech(a*x) + Log(x)/a + 1/a*Int(1/(x*(1 - a*x))*Sqrt((1 - a*x)/(1 + a*x)), x) <-- FreeQ(a, x))
Int(ℯ^ArcSech(a_'*x_^p_), x_Symbol) := (x*ℯ^ArcSech(a*x^p) + p/a*Int(1/x^p, x) + p*Sqrt(1 + a*x^p)/a*Sqrt(1/(1 + a*x^p))* Int(1/(x^p*Sqrt(1 + a*x^p)*Sqrt(1 - a*x^p)), x) <-- FreeQ([a, p], x))
Int(ℯ^ArcCsch(a_'*x_^p_'), x_Symbol) := (1/a*Int(1/x^p, x) + Int(Sqrt(1 + 1/(a^2*x^(2*p))), x) <-- FreeQ([a, p], x))
Int(ℯ^(n_'*ArcSech(u_)), x_Symbol) := (Int((1/u + Sqrt((1 - u)/(1 + u)) + 1/u*Sqrt((1 - u)/(1 + u)))^n, x) <-- IntegerQ(n))
Int(ℯ^(n_'*ArcCsch(u_)), x_Symbol) := (Int((1/u + Sqrt(1 + 1/u^2))^n, x) <-- IntegerQ(n))
Int(ℯ^ArcSech(a_'*x_^p_')/x_, x_Symbol) := (-1/(a*p*x^p) + Sqrt(1 + a*x^p)/a*Sqrt(1/(1 + a*x^p))* Int(Sqrt(1 + a*x^p)*Sqrt(1 - a*x^p)/x^(p + 1), x) <-- FreeQ([a, p], x))
Int(x_^m_'*ℯ^ArcSech(a_'*x_^p_'), x_Symbol) := (x^(m + 1)*ℯ^ArcSech(a*x^p)/(m + 1) + p/(a*(m + 1))*Int(x^(m - p), x) + p*Sqrt(1 + a*x^p)/(a*(m + 1))*Sqrt(1/(1 + a*x^p))* Int(x^(m - p)/(Sqrt(1 + a*x^p)*Sqrt(1 - a*x^p)), x) <-- FreeQ([a, m, p], x) && NeQ(m, -1))
Int(x_^m_'*ℯ^ArcCsch(a_'*x_^p_'), x_Symbol) := (1/a*Int(x^(m - p), x) + Int(x^m*Sqrt(1 + 1/(a^2*x^(2*p))), x) <-- FreeQ([a, m, p], x))
Int(x_^m_'*ℯ^(n_'*ArcSech(u_)), x_Symbol) := (Int(x^m*(1/u + Sqrt((1 - u)/(1 + u)) + 1/u*Sqrt((1 - u)/(1 + u)))^n, x) <-- FreeQ(m, x) && IntegerQ(n))
Int(x_^m_'*ℯ^(n_'*ArcCsch(u_)), x_Symbol) := (Int(x^m*(1/u + Sqrt(1 + 1/u^2))^n, x) <-- FreeQ(m, x) && IntegerQ(n))
Int(ℯ^(ArcSech(c_'*x_))/(a_ + b_'*x_^2), x_Symbol) := (1/(a*c)*Int(Sqrt(1/(1 + c*x))/(x*Sqrt(1 - c*x)), x) + 1/c*Int(1/(x*(a + b*x^2)), x) <-- FreeQ([a, b, c], x) && EqQ(b + a*c^2, 0))
Int(ℯ^(ArcCsch(c_'*x_))/(a_ + b_'*x_^2), x_Symbol) := (1/(a*c^2)*Int(1/(x^2*Sqrt(1 + 1/(c^2*x^2))), x) + 1/c*Int(1/(x*(a + b*x^2)), x) <-- FreeQ([a, b, c], x) && EqQ(b - a*c^2, 0))
Int((d_'*x_)^m_'*ℯ^(ArcSech(c_'*x_))/(a_ + b_'*x_^2), x_Symbol) := (d/(a*c)*Int((d*x)^(m - 1)*Sqrt(1/(1 + c*x))/Sqrt(1 - c*x), x) + d/c*Int((d*x)^(m - 1)/(a + b*x^2), x) <-- FreeQ([a, b, c, d, m], x) && EqQ(b + a*c^2, 0))
Int((d_'*x_)^m_'*ℯ^(ArcCsch(c_'*x_))/(a_ + b_'*x_^2), x_Symbol) := (d^2/(a*c^2)*Int((d*x)^(m - 2)/Sqrt(1 + 1/(c^2*x^2)), x) + d/c*Int((d*x)^(m - 1)/(a + b*x^2), x) <-- FreeQ([a, b, c, d, m], x) && EqQ(b - a*c^2, 0))
