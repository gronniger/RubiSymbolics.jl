...
If(TrueQ(DOLLARLoadShowSteps), Int(u_, x_Symbol) := (ShowStep("", "Int[a*u + b*v + ...,x]", "a*Integrate[u,x] + b*Integrate[v,x] + ...", Hold( IntSum(u, x))) <-- SimplifyFlag && SumQ(u)), Int(u_, x_Symbol) := (IntSum(u, x) <-- SumQ(u)))
